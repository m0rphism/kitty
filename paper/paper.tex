...
\begin{document}
  ...
  The operation \ACode{ϕ \& x} applies a map to a variable.
  The operation \ACode{ϕ ↑ s} lifts a map under a binder of sort \ACode{s}.
  The operation \ACode{⦅ x/t ⦆} constructs a singleton map that replaces
  \ACode{\ACon{zero}} with \ACode{x/t} and decreases all other variables by one.
  The \ACode{\AField{weaken s}} map increases all variables by one.
  The \ACode{ϕ₁ $\sim$ ϕ₂} type expresses extensional equality of maps.
    For simplicity, we postulate functional extensionality \ACode{$\sim$-ext}.\footnote{
      The actual implementation does \emph{not} use any postulates.
      % The workaround is explained in Appendix~\ref{sec:fun-ext}.
    }
  There is an identity map \ACode{id}.
  The lemma \ACode{id↑-id} states that a lifted identity map is again an identity map.
  ...
  map from \ACode{K₁} with a map from \ACode{K₂} that results in a map from
  \ACode{K₁⊔K₂}:
  \LibCode\KCKit
  ...
  instances for us. Hence, we define them in the record module of \ACode{\ADef{CTraversal}}:
  ...
  variable binders in the literature:
  de Bruijn\cite{scholar:de1972lambda},
  co de Bruijn\cite{DBLP:journals/corr/abs-1807-04085},
  ...
  used to ensure syntactic sorts, the semantic abstractions talk about untyped
  ...
  Our framework is able to automatically derive definitions and lemmas
  about untyped substitution, and provides an abstraction for proving
  type preservation of substitution with a single lemma (\ACode{\AField{\_⊢⋯\_}}).

  Similarly to frameworks with intrinsic typing, the main limitation in our work is the shape of
  typing relations: our typing kits can only model classical ternary typing relations.
  To adapt our framework to more complicated typing relations, the machinery for untyped substitution
  can be reused, but the typing kits and typing traversals need to be modified.
  We found that this works surprisingly well in practice,
  where we have already made extended typing kits to support
  linear typing ala Wood and Atkey\cite{DBLP:journals/corr/abs-2005-02247} and
  general substructural typing ala Licata et al\cite{DBLP:conf/rta/LicataSR17}.
  In both cases, the typing relation has a fourth component which models usage restrictions
  on the type context.
