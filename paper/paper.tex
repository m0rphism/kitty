...
\begin{document}
  ...
  where the subterm \texttt{e} is under a binder, we need to adjust \texttt{σ}
  for the shifted DeBruijn-indices in \texttt{e}:
  \begin{itemize}
  \item The variable \texttt{zero} refers in \texttt{e} to the variable bound by the binder.
  \item a variable \texttt{suc x} refers to variable \texttt{x} in \texttt{σ}.
  \end{itemize}
