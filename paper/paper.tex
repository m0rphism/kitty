...
\begin{document}
  ...
  % \begin{abstract}
  %   Formalizing a typed programming language in a proof assistant
  %   requires to choose between an extrinsic and an intrinsic
  %   representation of typing.
  %   \emph{Extrinsic} typing means to separately define a set of
  %   untyped terms and a typing relation.
  %   \emph{Intrinsic} typing means to directly define a set of well-typed terms,
  %   incorporating the typing relation into the syntax definition.

  %   One consequence of intrinsic typing is that operations on terms
  %   are also proofs that the operations preserve typing.
  %   This often leads to less code, as shared structure between
  %   definitions and preservation lemmas does not need to be repeated,
  %   but has the drawback that specification and proofs are
  %   intertwined, making it more difficult to check if a specification is
  %   correct.
  % \end{abstract}

  % \begin{abstract}
  %   Formalizing a typed programming language in a proof assistant
  %   requires to choose between an extrinsic and intrinsic representation of typing.
  %   For extrinsic typing, 
  %   There are two approaches to formalizing the syntax of typed object languages
  %   in proof assistants. The extrinsic approach defines
  %   object terms and typing relation separately, whereas the
  %   instrinsic approach only defines well-typed terms.

  %   This paper describes an Agda framework for deriving substitution
  %   operations and lemmas for syntaxes with extrinsic typing,
  %   intrinsic scoping, and DeBruijn Indices as variable representation.

  %   This paper describes an Agda framework for formalizing programming
  %   languages with extrinsic typing, intrinsic scoping, and DeBruijn
  %   Indices as variable representation. The key feature of the
  %   framework 
  %   all sorts of object variables.
  %   Agda's reflection mechanism is used to derive 
  %   substitution properties for a
  %   large class of syntaxes and only requires user code for to prove that the 

  %   framework is to deal with multiple sorts of variables in a uniform way
  % \end{abstract}

  % \begin{abstract}
  %   Variable binding and substitution are central concepts in the formalization
  %   of programming languages in proof assistants. 
  %   % Many different representations of variables have been developed with different
  %   % trade-offs on proof simplicity, clarity, and reduction of boilerplate.
  %   A common approach is to represent variables as DeBruijn Indices
  %   and to define substitution in terms of variable renamings for the sake
  ...
  to explicitly pass a particular Kit to those definitions:
  \begin{itemize}
  \item \ACode{Γ₂ \ADef{∋*/⊢*[} TK \ADef{]} ϕ \ADef{∶} Γ₁} is
    \ACode{Γ₂ \ADef{∋*/⊢*} ϕ \ADef{∶} Γ₁} for some \ACode{\ACon{TypingKit}} \ACode{TK}.
  \end{itemize}
  ...
  renamings/substitutions, it defines the type preservation of applying
