...
\begin{document}
  ...
  % TODO: avoiding interaction lemmas
  ...
  derive further operations:\footnote{
  ...
  % Before we define \ACode{\ACon{ComposeKit}} and
  % \ACode{\ACon{ComposeTraversal}}, we need one more ingredient.
  ...
  dependencies.
  While composition can be dervied, this has the benefit, that
  we've gained a new level of generality, which enables us in the next
  ...
  % \ACode{\ACon{ComposeTraversal}} and \ACode{\AField{⋯-fusion}} are analogous to
  % \ACode{\ACon{ComposeTraversal}} and \ACode{\AField{\_⋯\_}}.
  
  \subsection{An Abstraction for Composition}
  \label{sec:composition:ckit}

  In this subsection, we define the \ACode{\ACon{ComposeKit}}
  abstraction, which provides the \ACode{\AField{\_\&/⋯\_}} operation
  that is necessary for defining composition, and the
  \ACode{\ACon{ComposeTraversal}}, which provides the
  \ACode{\AField{⋯-fusion}} lemma for general maps.

  Similarly as it was the case for \ACode{\ACon{Kit}} and \ACode{\ACon{Traversal}},
  the \ACode{\ACon{ComposeTraversal}} needs to be instantiated for
  abstract \ACode{\ACon{ComposeKit}}s, which then provides the actual
  four \ACode{\ACon{ComposeKit}} instances. This allows us to break
  ...
  For this purpose, the \ACode{\ACon{ComposeKit}} abstraction, needs
  to satisfy two requirements:
  it needs to provide us with enough machinery to define composition, but also
  it needs to provide us with enough axioms that we are able to instanciate
  the \ACode{\ACon{ComposeTraversal}}. It is defined as followed:
  \LibCode\KComposeKit
  ...
  from the axioms of a \ACode{\ACon{ComposeKit}}:
  \begin{itemize}
  \item
  ...
  \end{itemize}
  To make it easier to explicitly specify a \ACode{\ACon{ComposeKit}},
  we define the notation \ACode{ϕ₁ \ADef{·[} C \ADef{]} ϕ₂}
  to stand for \ACode{ϕ₁ \ADef{·ₖ} ϕ₂} for some
  \ACode{\ACon{ComposeKit}} \ACode{C}.
  % \LibCode\KComposeKitNotation

  We then define the \ACode{\ACon{ComposeTraversal}} structure,
  which provides the \ACode{\AField{⋯-fusion}} lemma for the compositions
  described by arbitrary \ACode{\ACon{ComposeKit}}s:
  \LibCode\KComposeTraversal

  Similar as for the \ACode{\ACon{Traversal}}, we derive further
  definitions from the axiom of a \ACode{\ACon{ComposeTraversal}}:
  \begin{itemize}
  \item 
  ...
  \item 
    A singleton substitution followed by some map \ACode{ϕ} is the same
    as \ACode{ϕ} lifted over the variable of the singleton substitution
    followed by the singleton substitution with \ACode{ϕ} applied to
    its content:
  ...
  \item 
    We define \ACode{\ACon{ComposeKit}} instances for each of the four
    ...
    \ACode{\ADef{ComposeKit}} parameter.
    This allows us to first use \ACode{\ADef{Cᵣ}} as an argument,
    which yields the \ACode{\ADef{ComposeKit}} between substitutions
    and renamings, and then use this resulting \ACode{\ADef{ComposeKit}}
    as an argument for the case with two substitutions.
    As shown below, Agda's instance resolution is up for the task:

    % To get the \ACode{\ADef{ComposeKit}} between substitutions and
    % renamings, the argument \ACode{\ADef{Cᵣ}} is used.
    % The resulting \ACode{\ADef{ComposeKit}} is then again used as an argument
    % for \ACode{\ADef{Cₛ}} to yield the \ACode{\ADef{ComposeKit}}
  ...
  \subsection{Instantiation for System F}
  In this subsection, we show how to instantiate the
  \ACode{\ACon{ComposeTraversal}} abstraction for System F.
  In practice, this is done by our reflection algorithm automatically,
  ...
  \item
    ...
    provided by the \ACode{\ACon{ComposeKit}}.
  \item
  ...
  We start with \ACode{\ACon{TypingKit}}s, which abstract over
  variable and term \emph{typing}, and then define a
  \ACode{\ACon{TypingTraversal}}, which provides substitution
  preserves typing for all \ACode{\ACon{TypingKit}}s.
  \LibCode\KTypingKit
  The operations are completely analog to those of a \ACode{\ACon{Kit}}, but talk
  ...
  Given a \ACode{\ACon{TypingTraversal}}, we can easily instantiate the
  ...
  \subsection{Instantiation for System F}
  In this subsection, we show how to instantiate the
  \ACode{\ACon{TypingTraversal}} abstraction for System F.
  ...
  \item
    ...
    requiring us to use the \ACode{\AField{dist-↑-⦅⦆}} lemma.
  \end{itemize}
  Finally, we instantiate the \ACode{\ACon{TypingTraversal}} structure:
  ...
  We only listed the cases for $\beta$-reduction, as the other cases
  ...
  \item The beta-reduction rule for value-application is almost the same,
    ...
    expression substitution. This is a direct consequence of the weakening
    used in \ACode{\ACon{⊢λ}}.
  \end{itemize}
  ...
  \item
    ...
    \ACode{\ACon{TypingTraversal}}.
    Furthermore, the \ACode{\ACon{ComposeKit}} abstraction turned out
  ...
  \item
    ...
    the \ACode{\ACon{TypingKit}} abstraction.
    ...
    \ACode{\ACon{TypingKit}} machinery.
