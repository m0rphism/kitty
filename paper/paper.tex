...
\begin{document}
  ...
  For example, \ACon{[𝕥, 𝕥] ⊢ 𝕖} is the type of expressions (\ACon{𝕖}) with two free type-variables (\ACon{𝕥}).
  \newpage
  \AppCode\FSyntax
  The notation \ACon{\_⊢\_} is often used for terms in
  ...
  \ACode{S} to sorts with index \ACon{Var}.
  ...
  \AField{`\_} is injective, which is trivially true for
  ...
  in \ACode{e} are shifted, i.e.\ the variable \ACon{zero} refers
  ...
  to \ACon{\_∋\_} for the variable kit, and to \AField{\_⊢\_} for the
  term kit.

  A kit provides the following operations:
  \begin{itemize}
  \item 
    \AField{id/`} converts a variable \ACode{S \ACon{∋} s} into a
    ...
    variable constructor \AField{`\_}.
  \item 
    \AField{`/id} converts a \ACode{S ∋/⊢ s} into a term \ACode{S \AField{⊢} s}
    and is dual to the \AField{id/`} operation.
  \item 
    \AField{wk} shifts the DeBruin-Indices in a \ACode{S ∋/⊢ s}.
    The new, unused variable \ACon{zero} can assume any sort \ACode{s'}.
    For variables \AField{wk} is the successor \ACon{suc}.
    For terms \AField{wk} means applying a shifting renaming to the term.
  \end{itemize}

  The operations need to satisfy the following properties:
  \begin{itemize}
  \item \AField{`/`-is-`} states that converting a variable first to a ``variable-or-term'' and
    then further to a term is the same as converting it directly to a
    term using the variable constructor \AField{`\_}.
  \item \AField{`/id-injective} and \AField{id/`-injective} state that
    \AField{`/id} and \AField{id/`} are injective. This property
    follows easily, as both the identity function and the variable
    constructor are injective.
  \item \AField{wk-id/`} characterizes the behaviour of the \AField{wk} function.
  ...
  \item 
    ...
    \LibCode\KExt
  \item 
    A map can be shifted, as the Kit provides the necessary \AField{wk} operation:
    \LibCode\KWkm
  \item 
  ...
  \item 
    ...
    \LibCode\KId
  \item 
    There is a singleton map, which replaces \ACon{zero} and decreases
    all other variables by one:
    \LibCode\KSingle
  \item 
    There is a map which is equivalent to \AField{wk} on terms:
    \LibCode\KWeaken
  \end{itemize}
  ...
  raises a dependency problem: The \AField{wk} operation for terms requires us
