...
\begin{document}
  ...
  introduced an abstraction called \emph{Kits}.
  ...
  A kit is a structure that abstracts over the difference between variables and terms.
  There are always exactly two instances for Kits, one for variables and one for terms, 

  The core observation leading to this abstraction is that renamings
  \texttt{S₁ →ᵣ S₂}
  and substitutions \texttt{S₁ →ₛ S₂} only differ in their return type:
  while a substitution returns a term \texttt{S₂ ⊢ s}, a renaming returns
  a variable \texttt{S₂ ∋ s}.

  A Kit is a structure that abstracts precisely over the commonalities
