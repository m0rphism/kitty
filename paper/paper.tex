...
\begin{document}
  ...
  \begin{figure}
    ...
    \label{fig:map-ops}
  \end{figure}
  Figure~\ref{fig:map-ops} shows the usual operations for renamings and substitutions.\footnote{
  ...
  \item
    ...
    from the traversal operation, so we use \ACon{\AField{`/`-is-`}}.
  \item
  ...
  \item
    ...
    % TODO: use the notation def to the first time we use it?
  ...
  A \ACode{\ACon{CTraversal}} provides a generic \ACode{\AField{fusion}} lemma
  that works for any composition described by a \ACode{\ACon{CKit}}:
  \LibCode\KCTraversal

  Given a \ACode{\ACon{CTraversal}}, we can prove the lemmas about interactions
  of multiple maps, which are required for substitution preserves typing:
  \begin{itemize}
  \item 
