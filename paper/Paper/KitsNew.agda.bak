...
record Syntax : Set₁ where
  ...
  data RenSub : Set where Ren Sub : RenSub

  data Restrict : RenSub → S ⊢ s → Set where
    RRen : Restrict Ren (` x)
    RSub : Restrict Ren t 
  ...
  -- mutual
  --   wk : ∀ s' → (t : S ⊢ s) → Restrict r t → (s' ∷ S) ⊢ s
  --   wk _ .(` _) (RRen {x = x}) = ` suc x
  --   wk _ t      RSub           = {!!}
  ...
  _–[_]→_ : List (Sort Var) → RenSub → List (Sort Var) → Set
  S₁ –[ r ]→ S₂ = ∀ s → S₁ ∋ s → Σ (S₂ ⊢ s) (Restrict r)
  ...
  --       Cₛ : ⦃ K₂ : Kit _∋/⊢_ ⦄ ⦃ C : ComposeKit K₂ Kᵣ K₂ ⦄
  --            ⦃ W₂ : WkKit K₂ ⦄ → ComposeKit Kₛ K₂ Kₛ
  --       Cₛ = record
  --         { _&/⋯_     = _⋯_
  --         ; &/⋯-⋯     = λ t ϕ → refl
  --         ; &/⋯-wk-↑  = λ t ϕ → ⋯-↑-wk t ϕ _ }
