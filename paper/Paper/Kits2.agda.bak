...
record Syntax : Set₁ where
  ...
  record Bin (_∋/⊢_ : Scoped) : Set₁ where
    field
      binary : _∋/⊢_ ≡ _∋_ ⊎ _∋/⊢_ ≡ _⊢_
    ...
    id/`-injective with binary
    ... | inj₁ refl = λ { refl → refl }
    ... | inj₂ refl = `-injective
  ...
  record Kit (_∋/⊢_ : List (Sort Var) → Sort Var → Set) : Set₁ where
    ...
    field  wk              : ∀ s' → S ∋/⊢ s → (s' ∷ S) ∋/⊢ s
           wk-id/`         :  ∀ s' (x : S ∋ s) → wk s' (id/` x) ≡ id/` (suc x)

    module AltDefs2 where
      open AltDefs
  ...
  --         `/id (wk s' x/t) ≡ `/id x/t ⋯ weaken s'
    ...
    instance
      --! KitVar
      Kᵣ : Kit _∋_
      Kᵣ = record { bin      = record { binary = inj₁ refl }
                  ; wk       = λ s' x → suc x
                  ; wk-id/`  = λ s' x → refl }

      --! KitTerm
      Kₛ : Kit _⊢_
      Kₛ = record  { bin      = record { binary = inj₂ refl }
    ...
    wk-`/id : ∀ ⦃ K : Kit _∋/⊢_ ⦄ s {S s'} (x/t : S ∋/⊢ s') →
              `/id x/t ⋯ weaken ⦃ Kᵣ ⦄ s ≡ `/id (wk s x/t)
    wk-`/id ⦃ K ⦄ with Kit.binary K
    ... | inj₁ refl = λ s x → {!!}
    ... | inj₂ refl = λ s t → {!!}
