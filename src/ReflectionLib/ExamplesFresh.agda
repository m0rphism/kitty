...
Scoped : Set → Set → Set₁
Scoped VM TM = List VM → TM → Set

data Desc (VM : Set) (TM : Set) : Set₁ where
  `σ : (A : Set) → (A → Desc VM TM) → Desc VM TM
  `X : List VM → TM → Desc VM TM → Desc VM TM
  `■ : TM → Desc VM TM

⟦_⟧ : Desc VM TM → Scoped VM TM → Scoped VM TM
⟦ `σ A d     ⟧ X µ M = Σ[ a ∈ A ] (⟦ d a ⟧ X µ M)
⟦ `X µ' M' d ⟧ X µ M = X (µ' ++ µ) M' × ⟦ d ⟧ X µ M
⟦ `■ M'      ⟧ X µ M = M ≡ M'

data Tm (m→M : VM → TM) (d : Desc VM TM) : Scoped VM TM where
  `var : ∀ {µ m} → µ ∋ m → Tm m→M d µ (m→M m)
  `con : ∀ {µ M} → ⟦ d ⟧ (Tm m→M d) µ M → Tm m→M d µ M
...
deriveDesc : Name → Name → Name → Name → Name → TC ⊤
deriveDesc VarMode-nm TermMode-nm m→M-nm Term-nm desc-nm = runFreshT do
  VarMode  ← unquoteTC {A = Set} (def VarMode-nm [])
  TermMode ← unquoteTC {A = Set} (def TermMode-nm [])
      ...
      λ where
        (x , arg i arg-ty) desc → case unterm Term-nm arg-ty of λ where
          ...
          nothing → do
            desc ← unquoteTC' desc
            a ← unquoteTC' arg-ty
            quoteTC' (`σ {VM = VarMode} {TM = TermMode} a (λ _ → desc))
    pure $ clause [] [ argᵥ (fin-pat' i) ] desc
  f ← unquoteTC' (pat-lam clauses [])
  desc ← quoteNormTC' (`σ {VM = VarMode} {TM = TermMode} (Fin (length term-cons)) f)
  defdecFun'
    (argᵥ desc-nm)
    (def (quote Desc) [ argᵥ (def VarMode-nm []) ; argᵥ (def TermMode-nm []) ])
    [ clause [] [] desc ]

-- Derving To ------------------------------------------------------------------

deriveTo : Name → Name → Name → Name → Name → Name → TC ⊤
deriveTo VarMode-nm TermMode-nm m→M-nm Term-nm desc-nm to-nm = runFreshT $ do
  ty ← getDefinition Term-nm
  var-c , term-cs ← split-term-ctors $ ctors ty
  VarMode  ← unquoteTC {A = Set} (def VarMode-nm [])
  TermMode ← unquoteTC {A = Set} (def TermMode-nm [])
  m→M      ← unquoteTC {A = VarMode → TermMode} (def m→M-nm [])
  Term ← unquoteTC' {A = List VarMode → TermMode → Set} (def Term-nm [])
  d ← unquoteTC' (def desc-nm [])
  `VarMode ← quoteTC' VarMode
  `List-VarMode ← quoteTC' (List VarMode)
  ...
  to-ty ← quoteTC' (∀ {µ : List VarMode} {M : TermMode} → Term µ M → Tm m→M d µ M)
  defdecFun'
    (argᵥ to-nm)
    to-ty
...
deriveFrom : Name → Name → Name → Name → Name → Name → TC ⊤
deriveFrom VarMode-nm TermMode-nm m→M-nm Term-nm desc-nm from-nm = runFreshT $ do
  ty ← getDefinition Term-nm
  var-c , term-cs ← split-term-ctors $ ctors ty
  VarMode  ← unquoteTC {A = Set} (def VarMode-nm [])
  TermMode ← unquoteTC {A = Set} (def TermMode-nm [])
  ...
  term-clauses ← forM (enumerate term-cs) λ (i , c) → do
    ...
    let c-tel'' = List.map
          (λ { (x , arg i t) → case unterm Term-nm t of λ where
            (just (µ , M)) → (x , arg i (def (quote Tm) [ argᵥ (def m→M-nm []) ; argᵥ (def desc-nm []) ; argᵥ µ ; argᵥ M ]))
            nothing        → (x , arg i t)
          })
          c-tel
    pure $ clause
      ...
      (con c ts)
  from-ty ← quoteTC' (∀ {µ : List VarMode} {M : TermMode} → Tm m→M d µ M → Term µ M)
...
deriveFromTo : Name → Name → Name → Name → Name → Name → Name → Name → TC ⊤
deriveFromTo VarMode-nm TermMode-nm m→M-nm Term-nm desc-nm to-nm from-nm from∘to-nm = runFreshT $ do
  ty ← getDefinition Term-nm
  let cs = ctors ty
  ...
  m→M      ← unquoteTC {A = VarMode → TermMode} (def m→M-nm [])
  Term     ← unquoteTC {A = List VarMode → TermMode → Set} (def Term-nm [])
  d        ← unquoteTC {A = Desc VarMode TermMode} (def desc-nm [])
  from     ← unquoteTC {A = ∀ {µ M} → Tm m→M d µ M → Term µ M} (def from-nm [])
  to       ← unquoteTC {A = ∀ {µ M} → Term µ M → Tm m→M d µ M} (def to-nm [])
  `List-VarMode ← quoteTC' (List VarMode)
  `VarMode ← quoteTC' VarMode
  let var-clause = clause
        [ ("µ" , argₕ `List-VarMode)
  ...
  from∘to-ty ← quoteTC' (∀ {µ : List VarMode} {M : TermMode} → (x : Term µ M) → from (to x) ≡ x)
  defdecFun' (argᵥ from∘to-nm) from∘to-ty (var-clause ∷ term-clauses)

-- Deriving to∘from ------------------------------------------------------------

deriveToFromClause : Name → Name → Name → Name → Name → Name → Name → ℕ → FreshT TC Clause'
deriveToFromClause VarMode-nm TermMode-nm m→M-nm Term-nm desc-nm to∘from-nm con-nm con-i = do
  c-ty ← getType' con-nm
  let c-tel , c-ret = pi→tel c-ty
  let c-tel' = List.drop 1 c-tel
  `µ ← case List.head c-tel of λ where
...
deriveToFrom : Name → Name → Name → Name → Name → Name → Name → Name → TC ⊤
deriveToFrom VarMode-nm TermMode-nm m→M-nm Term-nm desc-nm to-nm from-nm to∘from-nm = runFreshT $ do
  ty ← getDefinition Term-nm
  let cs = ctors ty
  var-c , term-cs ← split-term-ctors cs
  VarMode  ← unquoteTC {A = Set} (def VarMode-nm [])
  ...
  let var-clause = clause
        ...
        (con (quote refl) [])
  term-clauses ← forM (enumerate term-cs) λ (i , c) → do
    deriveToFromClause VarMode-nm TermMode-nm m→M-nm Term-nm desc-nm to∘from-nm c i
  to∘from-ty ← quoteTC' (∀ {µ : List VarMode} {M : TermMode} → (x : Tm m→M d µ M) → to (from x) ≡ x)
  defdecFun' (argᵥ to∘from-nm) to∘from-ty (var-clause ∷ term-clauses)

-- Deriving the Isomorphism ----------------------------------------------------

deriveIso : Name → Name → Name → Name → Name → TC ⊤
deriveIso VarMode-nm TermMode-nm m→M-nm Term-nm Iso-nm = do
  printAST "–– deriveIso –––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––––"
  desc-nm    ← freshName "d"
  to-nm      ← freshName "to"
  ...
  deriveDesc   VarMode-nm TermMode-nm m→M-nm Term-nm desc-nm
  deriveTo     VarMode-nm TermMode-nm m→M-nm Term-nm desc-nm to-nm
  deriveFrom   VarMode-nm TermMode-nm m→M-nm Term-nm desc-nm from-nm
  deriveFromTo VarMode-nm TermMode-nm m→M-nm Term-nm desc-nm to-nm from-nm from∘to-nm
  deriveToFrom VarMode-nm TermMode-nm m→M-nm Term-nm desc-nm to-nm from-nm to∘from-nm
  VarMode  ← unquoteTC {A = Set} (def VarMode-nm [])
  TermMode ← unquoteTC {A = Set} (def TermMode-nm [])
  m→M      ← unquoteTC {A = VarMode → TermMode} (def m→M-nm [])
  Term     ← unquoteTC {A = List VarMode → TermMode → Set} (def Term-nm [])
  d        ← unquoteTC {A = Desc VarMode TermMode} (def desc-nm [])
  iso-ty   ← quoteTC (∀ {µ} {M} → (Term µ M) ≃ Tm m→M d µ M)
  defdecFun
    (argᵥ Iso-nm)
    iso-ty
...
deriveIso' : (VarMode : Set)
           → (TermMode : Set)
           → (VarMode → TermMode)
           → (List VarMode → TermMode → Set)
           → Name
           → TC ⊤
deriveIso' VarMode TermMode m→M Term Iso-nm = do
  VarMode-nm  ← type→name =<< runFreshT (quoteTC' VarMode)
  TermMode-nm ← type→name =<< runFreshT (quoteTC' TermMode)
  m→M-nm      ← type→name =<< runFreshT (quoteTC' m→M)
  Term-nm     ← type→name =<< runFreshT (quoteTC' Term)
  deriveIso VarMode-nm TermMode-nm m→M-nm Term-nm Iso-nm
...
-- unquoteDecl Iso = deriveIso (quote VarMode) (quote TermMode) (quote m→M) (quote _⊢_) Iso
unquoteDecl Iso = deriveIso' VarMode TermMode m→M _⊢_ Iso
