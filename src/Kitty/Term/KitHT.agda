...
module Kitty.Term.KitHT {𝕄 : Modes} (𝕋 : Terms 𝕄) (T : Traversal 𝕋) (𝕊 : Kitty.Term.Sub.SubWithLaws 𝕋) where
...
record KitHomotopy : Set₁ where
  ...
  ⋯-ι-→ :
    ∀ ⦃ 𝕂₁ 𝕂₂ : KitT ⦄ ⦃ 𝕂₁⊑𝕂₂ : 𝕂₁ ⊑ₖ 𝕂₂ ⦄ ⦃ 𝕊 : SubWithLaws ⦄
      {µ₁ µ₂ M}
      (t : µ₁ ⊢ M) (ϕ : µ₁ –[ 𝕂₁ ]→ µ₂)
    → t ⋯ ϕ ≡ t ⋯ ι-→ ϕ
  ⋯-ι-→ t ϕ = ~-cong-⋯ t (~-ι-→ ϕ)

  ren→sub : ∀ ⦃ 𝕊 : SubWithLaws ⦄ {µ₁ µ₂ M} (t : µ₁ ⊢ M) (ρ : µ₁ →ᵣ µ₂) →
