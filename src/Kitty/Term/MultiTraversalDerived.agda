...
module WithSub {ℓ} (S : Kitty.Term.Sub.SubWithLaws 𝕋 ℓ) where
  ...
  private
    ⋯-id' :
      ∀ {M} {_∋/⊢_ : Scoped M} ⦃ 𝕂 : Kit _∋/⊢_ ⦄ {µ M} (v : µ ⊢ M) →
      v ⋯-MT id ⦃ 𝕂 = 𝕂 ⦄ ≡ v
    ⋯-id' ⦃ 𝕂 ⦄ {µ} {M} v =
      ⋯-↑ ⦃ 𝕊 = S ⦄ {µ₁ = µ} (id ∷[ (_ , _ , 𝕂) ] [])
          []
          (λ {µ} x →
            ...
            `/id (x & (id ⦃ 𝕂 = 𝕂 ⦄ ↑*' µ)) ≡⟨ use-~ (↑*'~↑* ⦃ 𝕊 = S ⦄ µ) _ x ⟩
            `/id (x & (id ⦃ 𝕂 = 𝕂 ⦄ ↑* µ))  ≡⟨ use-~ (id↑*~id µ _) _ x ⟩
        ...
        v ⋯* (g ∷[ (_ , _ , 𝕂₂) ] f ∷[ (_ , _ , 𝕂₁) ] [])
          ≡⟨ ⋯-↑ ⦃ 𝕊 = S ⦄
                ...
                (λ {µ} x →
                  ...
                  `/id (x & ((f ↑* µ) ·ₖ (g ↑* µ)))  ≡⟨ sym (use-~ (dist-↑*-· µ f g) _ x) ⟩
                  `/id (x & ((f ·ₖ g) ↑* µ))         ≡⟨ sym (⋯-var x ((g ∘ₖ f) ↑* µ)) ⟩
                  ` x ⋯ (f ·ₖ g) ↑* µ                ≡⟨ sym (~-cong-⋯ (` x) (↑*'~↑* ⦃ 𝕊 = S ⦄ µ)) ⟩
                  ` x ⋯ (f ·ₖ g) ↑*' µ               ∎)
                v
          ⟩
        v ⋯* (_∷_ {b = _ , _ , 𝕂} (g ∘ₖ f) [])       ≡⟨ refl ⟩
        v ⋯ (g ∘ₖ f)       ∎
