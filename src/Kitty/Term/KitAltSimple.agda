...
record KitTraversalAlt : Set₁ where
  ...
  -- ~-cong-⋯ :
  --   ∀ ⦃ 𝕂₁ 𝕂₂ : Kit ⦄ ⦃ 𝕊 : SubWithLaws ⦄ {f : µ₁ –[ 𝕂₁ ]→ µ₂} {g : µ₁ –[ 𝕂₂ ]→ µ₂} (t : µ₁ ⊢ M)
  --   → f ~ g
  --   → t ⋯ f ≡ t ⋯ g
...
module Derive (KT : KitTraversalAlt) where
  ...
  open Traversal kit-traversal hiding (_⋯_; ⋯-var; kitᵣ; _≈ₜ_; _≈ₓ_; _⋯*_; subst-⋯) public

  private instance _ = kitᵣ; _ = kitₛ

  ⋯-x/t-wk'ₛ-var : ∀ ⦃ 𝕂 ⦄ ⦃ 𝕊 : SubWithLaws ⦄ {m'} {µ} {mx} (x : (µ₁ ▷▷ µ) ∋ mx)
              → ` x ⋯ wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id ↑*' µ ≡ ` x ⋯ wkₖ ⦃ 𝕂 = kitᵣ ⦄ m' id ↑*' µ
  -- ⋯-x/t-wk'ₛ-var {µ₁} ⦃ 𝕂 ⦄ {m'} {µ} {mx} x = {!!}
  ⋯-x/t-wk'ₛ-var {µ₁} ⦃ 𝕂 ⦄ {m'} {[]} {mx} x =
    (` x) ⋯ wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id       ≡⟨ ⋯-var x (wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id) ⟩
    `/id (x & wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id)    ≡⟨ {!!} ⟩
    `/id (x & wkₖ ⦃ 𝕂 = kitᵣ ⦄ m' id) ≡⟨ {!!} ⟩
    (` x) ⋯ wkₖ ⦃ 𝕂 = kitᵣ ⦄ m' id    ∎
  ⋯-x/t-wk'ₛ-var {µ₁} ⦃ 𝕂 ⦄ {m'} {µ ▷ m} {mx} x@(here refl) =
    (` x) ⋯ wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id ↑*' (µ ▷ m)     ≡⟨ refl ⟩
    (` x) ⋯ wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id ↑*' µ ↑ m       ≡⟨ ⋯-var x (wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id ↑*' µ ↑ m) ⟩
    `/id (x & wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id ↑*' µ ↑ m)    ≡⟨ {!!} ⟩
    `/id (x & wkₖ ⦃ 𝕂 = kitᵣ ⦄ m' id ↑*' µ ↑ m) ≡⟨ sym (⋯-var x (wkₖ ⦃ 𝕂 = kitᵣ ⦄ m' id ↑*' µ ↑ m)) ⟩
    (` x) ⋯ wkₖ ⦃ 𝕂 = kitᵣ ⦄ m' id ↑*' µ ↑ m    ≡⟨⟩
    (` x) ⋯ wkₖ ⦃ 𝕂 = kitᵣ ⦄ m' id ↑*' (µ ▷ m)    ∎
  ⋯-x/t-wk'ₛ-var {µ₁} ⦃ 𝕂 ⦄ {m'} {µ ▷ m} {mx} x@(there y) =
    (` x) ⋯ wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id ↑*' (µ ▷ m)     ≡⟨ refl ⟩
    (` x) ⋯ wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id ↑*' µ ↑ m       ≡⟨ ⋯-var x (wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id ↑*' µ ↑ m) ⟩
    `/id (x & wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id ↑*' µ ↑ m)    ≡⟨ {!!} ⟩
    `/id (wk ⦃ 𝕂 ⦄ m (y & wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id ↑*' µ)) ≡⟨ {!!} ⟩
    `/id (y & wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id ↑*' µ) ⋯ wkₖ ⦃ 𝕂 = kitᵣ ⦄ m id ≡⟨ {!!} ⟩
    wk ⦃ kitₛ ⦄ m (`/id (y & wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id ↑*' µ)) ≡⟨ {!!} ⟩
    wk ⦃ kitₛ ⦄ m (`/id (y & wkₖ ⦃ 𝕂 = kitᵣ ⦄ m' id ↑*' µ)) ≡⟨ {!!} ⟩
    `/id (wk ⦃ kitᵣ ⦄ m (y & wkₖ ⦃ 𝕂 = kitᵣ ⦄ m' id ↑*' µ)) ≡⟨ {!!} ⟩
    `/id (x & wkₖ ⦃ 𝕂 = kitᵣ ⦄ m' id ↑*' µ ↑ m) ≡⟨ sym (⋯-var x (wkₖ ⦃ 𝕂 = kitᵣ ⦄ m' id ↑*' µ ↑ m)) ⟩
    (` x) ⋯ wkₖ ⦃ 𝕂 = kitᵣ ⦄ m' id ↑*' µ ↑ m    ≡⟨⟩
    (` x) ⋯ wkₖ ⦃ 𝕂 = kitᵣ ⦄ m' id ↑*' (µ ▷ m)    ∎

  ⋯-x/t-wk'ₛ : ∀ ⦃ 𝕂 ⦄ ⦃ 𝕊 : SubWithLaws ⦄ {m'} {M} (t : µ₁ ⊢ M)
              → t ⋯ wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id ≡ wk ⦃ kitₛ ⦄ m' t
  ⋯-x/t-wk'ₛ {µ₁} ⦃ 𝕂 ⦄ {m'} {M} t =
    t ⋯ wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' (id ⦃ 𝕂 = 𝕂 ⦄)
      ≡⟨ ⋯-↑ {µ₁ = µ₁} (wkₖ ⦃ 𝕂 = 𝕂 ⦄ m' id ∷ [])
             (wkₖ ⦃ 𝕂 = kitᵣ ⦄ m' id ∷ [])
