...
record SubCompose : Set (lsuc ℓ) where
  ...
  field
    _·ₖ_ :
      ∀ ⦃ 𝕂₁ 𝕂₂ 𝕂 ⦄ ⦃ C : ComposeKit 𝕂₁ 𝕂₂ 𝕂 ⦄ {µ₁ µ₂ µ₃}
      → µ₁ –[ 𝕂₁ ]→ µ₂
      → µ₂ –[ 𝕂₂ ]→ µ₃
      → µ₁ –[ 𝕂  ]→ µ₃
  ...
  ~-cong-· ⦃ 𝕂₁ ⦄ ⦃ 𝕂₂ ⦄ ⦃ 𝕂 ⦄ ⦃ C ⦄ {µ₁} {µ₂} {µ₃} {ϕ₁} {ϕ₁'} {ϕ₂} {ϕ₂'} ϕ₁~ϕ₁' ϕ₂~ϕ₂' m x =
    ...
    `/id (x & ϕ₁  &/⋯ ϕ₂ )  ≡⟨ cong `/id (~-cong-&/⋯ (~→~' ϕ₁~ϕ₁' _ x) ϕ₂~ϕ₂') ⟩
  ...
  dist-↑-· {µ₁} {µ₂} {µ₃} m ϕ₁ ϕ₂ mx x@(here refl) =
    ...
    `/id (id/` (here refl))              ≡⟨ ι-`/id (id/` (here refl)) ⟩
    `/id (ι-∋/⊢ (id/` (here refl)))      ≡⟨ cong (λ ■ → `/id (ι-∋/⊢ ■)) (sym (&-↑-here ϕ₂)) ⟩
    `/id (ι-∋/⊢ (here refl & (ϕ₂ ↑ m)))  ≡⟨ cong `/id (sym (&/⋯-& (here refl) (ϕ₂ ↑ m))) ⟩
    `/id (id/` (here refl) &/⋯ (ϕ₂ ↑ m)) ≡⟨ cong (λ ■ → `/id (■ &/⋯ ϕ₂ ↑ m)) (sym (&-↑-here ϕ₁)) ⟩
  ...
  dist-↑*-· :
    ∀ ⦃ 𝕂₁ 𝕂₂ 𝕂₁⊔𝕂₂ ⦄ ⦃ C : ComposeKit 𝕂₁ 𝕂₂ 𝕂₁⊔𝕂₂ ⦄ {µ₁ µ₂ µ₃}
      µ (ϕ₁ : µ₁ –[ 𝕂₁ ]→ µ₂) (ϕ₂ : µ₂ –[ 𝕂₂ ]→ µ₃)
    → ((ϕ₁ ·ₖ ϕ₂) ↑* µ) ~ ((ϕ₁ ↑* µ) ·ₖ (ϕ₂ ↑* µ))
  dist-↑*-· []      ϕ₁ ϕ₂ =
    ((ϕ₁ ·ₖ ϕ₂) ↑* [])                  ~⟨ ↑*-[] (ϕ₁ ·ₖ ϕ₂) ⟩
  ...
  ·-idˡ :
    ∀ ⦃ 𝕂₁ 𝕂₂ 𝕂 ⦄ ⦃ C : ComposeKit 𝕂₁ 𝕂₂ 𝕂 ⦄ {µ₁} {µ₂} (ϕ : µ₁ –[ 𝕂₂ ]→ µ₂)
    → (id ⦃ 𝕂 = 𝕂₁ ⦄ ·ₖ ϕ) ~ ϕ
