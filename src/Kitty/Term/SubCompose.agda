...
record SubCompose : Set (lsuc ℓ) where
  ...
  wk-↓ ⦃ 𝕂₁ ⦄ ⦃ 𝕂₂ ⦄ ⦃ 𝕂 ⦄ ⦃ C ⦄ {µ₁} {µ₂} {m₁} ϕ m x =
    ...
    Kit.`/id 𝕂 (sub (x & wkₖ ⦃ 𝕂 = 𝕂₁ ⦄ m₁ id &/⋯[ C ] ϕ))         ≡⟨ cong (λ ■ → Kit.`/id 𝕂 (sub (■ &/⋯[ C ] ϕ)))
                                                                            (&-wkₖ-wk id x) ⟩
    Kit.`/id 𝕂 (sub (Kit.wk 𝕂₁ _ (x & id ⦃ 𝕂 = 𝕂₁ ⦄) &/⋯[ C ] ϕ))  ≡⟨ cong (λ ■ → Kit.`/id 𝕂 (sub (Kit.wk 𝕂₁ _ ■ &/⋯[ C ] ϕ)))
                                                                             (&-id x) ⟩
    Kit.`/id 𝕂 (sub (Kit.wk 𝕂₁ _ (Kit.id/` 𝕂₁ x) &/⋯[ C ] ϕ))      ≡⟨ cong (λ ■ → Kit.`/id 𝕂 (sub (■ &/⋯[ C ] ϕ)))
                                                                            (Kit.wk-id/` 𝕂₁ _ x) ⟩
    Kit.`/id 𝕂 (sub (Kit.id/` 𝕂₁ (there x) &/⋯[ C ] ϕ))            ≡⟨ cong (Kit.`/id 𝕂) (&/⋯-& (there x) ϕ) ⟩
    Kit.`/id 𝕂 (sub' (ι-∋/⊢ ⦃ 𝕂₂⊑𝕂₁⊔𝕂₂ ⦃ C ⦄ ⦄ (there x & ϕ)))     ≡⟨ ι-∋/⊢-~ₜ (there x & ϕ) ⟩
