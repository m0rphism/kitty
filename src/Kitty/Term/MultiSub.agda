...
_–[_]→*_ : ∀ ⦃ 𝕊 : Sub ℓ ⦄ → List VarMode → (_ : List Kit) → List VarMode → Set (ℓ ⊔ lsuc 0ℓ)
µ₁ –[ 𝕂s ]→* µ₂ = Star (λ 𝕂 x y → y –[ 𝕂 ]→ x) 𝕂s µ₂ µ₁

infixl  11  _↑*'_
_↑*'_ : ∀ ⦃ 𝕊 : Sub ℓ ⦄ ⦃ 𝕂 ⦄ {µ₁} {µ₂} → µ₁ –[ 𝕂 ]→ µ₂ → ∀ µ' → (µ₁ ▷▷ µ') –[ 𝕂 ]→ (µ₂ ▷▷ µ')
f ↑*' []      = f
f ↑*' (µ ▷ m) = f ↑*' µ ↑ m

~-cong-↑*'' : ∀ ⦃ 𝕊 : SubWithLaws ℓ ⦄ ⦃ 𝕂 : Kit ⦄ {µ₁} {µ₂} {µ} {ϕ : µ₁ –[ 𝕂 ]→ µ₂} {ϕ' : µ₁ –[ 𝕂 ]→ µ₂} →
  ϕ ~ ϕ' →
  (ϕ ↑*' µ) ~ (ϕ' ↑*' µ)
~-cong-↑*'' {µ = []}    ϕ~ϕ' = ϕ~ϕ'
~-cong-↑*'' {µ = µ ▷ m} ϕ~ϕ' = ~-cong-↑' (~-cong-↑*'' ϕ~ϕ')

↑*'~↑* : ∀ ⦃ 𝕊 : SubWithLaws ℓ ⦄ ⦃ 𝕂 : Kit ⦄ {µ₁} {µ₂} {ϕ : µ₁ –[ 𝕂 ]→ µ₂} µ → ϕ ↑*' µ ~ ϕ ↑* µ
↑*'~↑* ⦃ 𝕊 ⦄ ⦃ 𝕂 ⦄ {µ₁} {µ₂} {ϕ} [] mx x =
  `/id (x & ϕ ↑*' []) ≡⟨⟩
  `/id (x & ϕ)        ≡⟨ sym (↑*-[] ϕ _ x) ⟩
  `/id (x & ϕ ↑*  [])  ∎
...
infixl  11  _↑**_
_↑**_ : ∀ ⦃ 𝕊 : Sub ℓ ⦄ {𝕂s : List Kit} → µ₁ –[ 𝕂s ]→* µ₂ → ∀ µ' → (µ' ++ µ₁) –[ 𝕂s ]→* (µ' ++ µ₂)
[] ↑** µ' = []
(_∷_ {b = 𝕂} f fs) ↑** µ' = (_↑*'_ ⦃ 𝕂 = 𝕂 ⦄ f µ') ∷ (fs ↑** µ')

↑**-[] :
  ∀ ⦃ 𝕊 : SubWithLaws ℓ ⦄ {𝕂s : List Kit} {µ₁ µ₂} (fs : µ₁ –[ 𝕂s ]→* µ₂)
  → fs ↑** [] ≡ fs
↑**-[] [] = refl
↑**-[] (f ∷ fs) = cong (f ∷_) (↑**-[] fs)

dist-↑*'-▷▷ :
  ∀ ⦃ 𝕊 : SubWithLaws ℓ ⦄ {{𝕂 : Kit}} µ' µ''
  → (f : µ₁ –[ 𝕂 ]→ µ₂)
...
dist-↑**-▷▷ :
  ∀ ⦃ 𝕊 : SubWithLaws ℓ ⦄ {𝕂s : List Kit} µ' µ''
  → (f : µ₁ –[ 𝕂s ]→* µ₂)
...
dist-↑**-▷▷ {µ₁} {µ₂} {𝕂s = 𝕂s} µ' []        f =
  ...
  f ↑** µ'         ≡⟨⟩
  f ↑** (µ' ▷▷ []) ∎
dist-↑**-▷▷ {ℓ} {µ₁} {.µ₁} µ' (µ'' ▷ m) []       = subst-[]-flip (λ 𝕂s µ₂ µ₁ → µ₁ –[ 𝕂s ]→ µ₂) (cong (_∷_ m) (++-assoc µ'' µ' µ₁))
dist-↑**-▷▷ {ℓ} {µ₁} {µ₂} {𝕂 ∷ 𝕂s}  µ' (µ'' ▷ m) (_∷_ {a₁ = .µ₂} {a₂ = y} f fs) =
...
module TraversalOps (_⋯_ : ∀ {ℓ} ⦃ 𝕂 : Kit ⦄ ⦃ 𝕊 : Sub ℓ ⦄ {µ₁} {µ₂} {M} → µ₁ ⊢ M → µ₁ –[ 𝕂 ; 𝕊 ]→ µ₂ → µ₂ ⊢ M) where
  infixl  8 _⋯*_
  _⋯*_ : ∀ ⦃ 𝕊 : Sub ℓ ⦄ {𝕂s : List Kit} {µ₁ µ₂ M} →
        µ₁ ⊢ M → µ₁ –[ 𝕂s ]→* µ₂ → µ₂ ⊢ M
  t ⋯* fs = fold-star' (λ 𝕂 _ _ t f → _⋯_ {{𝕂}} t f) t fs

  _≈ₓ_ : ∀ ⦃ 𝕊 : Sub ℓ ⦄ {𝕂s₁ 𝕂s₂ : List Kit} {µ₁ µ₂} → (f : µ₁ –[ 𝕂s₁ ]→* µ₂) → (g : µ₁ –[ 𝕂s₂ ]→* µ₂) → Set
  _≈ₓ_ {µ₁ = µ₁} f g = ∀ {µ₁'} {m} (x : (µ₁ ▷▷ µ₁') ∋ m) → (` x) ⋯* (f ↑** µ₁') ≡ (` x) ⋯* (g ↑** µ₁')

  _≈ₜ_ : ∀ ⦃ 𝕊 : Sub ℓ ⦄ {𝕂s₁ 𝕂s₂ : List Kit} {µ₁ µ₂} → (f : µ₁ –[ 𝕂s₁ ]→* µ₂) → (g : µ₁ –[ 𝕂s₂ ]→* µ₂) → Set
  _≈ₜ_ {µ₁ = µ₁} f g = ∀ {µ₁'} {M} (t : (µ₁ ▷▷ µ₁') ⊢ M) → t ⋯* (f ↑** µ₁') ≡ t ⋯* (g ↑** µ₁')

  subst-⋯ :
    ∀ ⦃ 𝕊 : Sub ℓ ⦄ {𝕂s : List Kit} {µ₁ µ₂ µ₁' µ₂'}
      (f : µ₁ –[ 𝕂s ]→* µ₂) {M} (t : µ₁' ⊢ M)
    → (µ₁≡µ₁' : µ₁ ≡ µ₁')
    → (µ₂≡µ₂' : µ₂ ≡ µ₂')
  ...
  lemy :
    ∀ ⦃ 𝕊 : SubWithLaws ℓ ⦄ {𝕂s : List Kit} {µ₁ µ₂ µ' µ''}
  ...
  ≈↑** :
    ∀ ⦃ 𝕊 : SubWithLaws ℓ ⦄ {𝕂s₁ 𝕂s₂ : List Kit} {µ₁ µ₂}
      (f : µ₁ –[ 𝕂s₁ ]→* µ₂) (g : µ₁ –[ 𝕂s₂ ]→* µ₂)
    → (∀ {µ₁'} {m} (x : (µ₁ ▷▷ µ₁') ∋ m)
        → ((` x) ⋯* (f ↑** µ₁')) ≡ ((` x) ⋯* (g ↑** µ₁')))
    → (∀ {µ₁'} {µ₁''} {m} (x : (µ₁ ▷▷ µ₁' ▷▷ µ₁'') ∋ m)
...
open import Data.Unit using (⊤; tt)
module TraversalOps' (_⋯_ : ⊤ → ∀ {ℓ} ⦃ 𝕊 : Sub ℓ ⦄ ⦃ 𝕂 : Kit ⦄ {µ₁} {µ₂} {M} → µ₁ ⊢ M → µ₁ –[ 𝕂 ; 𝕊 ]→ µ₂ → µ₂ ⊢ M) where
