...
↑**-[] :
  ∀ ⦃ 𝕊 : SubWithLaws ⦄ {𝕂s : List Kit} {µ₁ µ₂} (fs : µ₁ –[ 𝕂s ]→* µ₂)
  → fs ↑** [] ≡ fs
↑**-[] [] = refl
↑**-[] (f ∷ fs) = cong (f ∷_) (↑**-[] fs)

dist-↑*'-▷▷ :
  ∀ ⦃ 𝕊 : SubWithLaws ⦄ {{𝕂 : Kit}} µ' µ''
  → (f : µ₁ –[ 𝕂 ]→ µ₂)
  → let sub = subst₂ (_–[ 𝕂 ]→_) (++-assoc µ'' µ' µ₁) (++-assoc µ'' µ' µ₂) in
    f ↑*' µ' ↑*' µ'' ≡ sub (f ↑*' (µ' ▷▷ µ''))
dist-↑*'-▷▷ {µ₁} {µ₂} µ' []        f = refl
dist-↑*'-▷▷ {µ₁} {µ₂} ⦃ 𝕊 ⦄ ⦃ 𝕂 ⦄ µ' (µ'' ▷ m) f =
...
dist-↑**-▷▷ {µ₁} {µ₂} {𝕂 ∷ 𝕂s}  µ' (µ'' ▷ m) (_∷_ {a₁ = .µ₂} {a₂ = y} f fs) =
...
module TraversalOps (_⋯_ : ∀ ⦃ 𝕂 : Kit ⦄ ⦃ 𝕊 : Sub ⦄ {µ₁} {µ₂} {M} → µ₁ ⊢ M → µ₁ –[ 𝕂 ; 𝕊 ]→ µ₂ → µ₂ ⊢ M) where
  ...
  lemy :
    ∀ ⦃ 𝕊 : SubWithLaws ⦄ {𝕂s : List Kit} {µ₁ µ₂ µ' µ''}
  ...
  ≈↑** :
    ∀ ⦃ 𝕊 : SubWithLaws ⦄ {𝕂s₁ 𝕂s₂ : List Kit} {µ₁ µ₂}
      (f : µ₁ –[ 𝕂s₁ ]→* µ₂) (g : µ₁ –[ 𝕂s₂ ]→* µ₂)
    → (∀ {µ₁'} {m} (x : (µ₁ ▷▷ µ₁') ∋ m)
        → ((` x) ⋯* (f ↑** µ₁')) ≡ ((` x) ⋯* (g ↑** µ₁')))
    → (∀ {µ₁'} {µ₁''} {m} (x : (µ₁ ▷▷ µ₁' ▷▷ µ₁'') ∋ m)
