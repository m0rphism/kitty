...
module Private where
  ...
  ~-wkᵣ ⦃ 𝕂₂ ⦄ {µ} {m} {mx} {x₁} {x/t₂} eq =
    ...
    `/id ⦃ 𝕂₂ ⦄ (wk ⦃ 𝕂₂ ⦄ m (id/` x₁)) ≡⟨ cong (λ ■ → `/id ⦃ 𝕂₂ ⦄ (wk ⦃ 𝕂₂ ⦄ m ■))
...
wk-`/id :
  ∀ ⦃ 𝕂 : Kit _∋/⊢_ ⦄ ⦃ KT : KitT 𝕂 ⦄ {µ} m {mx}
    (x/t : µ ∋/⊢[ 𝕂 ] id/m→M mx)
  → wk m (`/id x/t) ≡ `/id (wk m x/t)
wk-`/id ⦃ KT = KT ⦄ m x/t = KitK.wkₖ-⋯ (KitT.KitT-KitK KT)

~-wk :
  ∀ ⦃ 𝕂₁ : Kit _∋/⊢₁_ ⦄ ⦃ 𝕂₂ : Kit _∋/⊢₂_ ⦄ ⦃ KT₁ : KitT 𝕂₁ ⦄ ⦃ KT₂ : KitT 𝕂₂ ⦄ {µ} {m} {mx}
    {x/t₁ : µ ∋/⊢[ 𝕂₁ ] id/m→M mx}
    {x/t₂ : µ ∋/⊢[ 𝕂₂ ] id/m→M mx}
  → `/id x/t₁ ≡ `/id x/t₂
  → `/id (wk m x/t₁) ≡ `/id (wk m x/t₂)
...
~-cong-wk :
  ...
  ϕ ~ ϕ' →
  wkₖ m ϕ ~ wkₖ m ϕ'
~-cong-wk ⦃ 𝕂₁ ⦄ ⦃ 𝕂₂ ⦄ ⦃ W₁ ⦄ ⦃ W₂ ⦄ {µ₁} {µ₂} {m} {ϕ} {ϕ'} ϕ~ϕ' = mk-~ λ mx x →
...
↑*-▷▷ :
  ∀ ⦃ 𝕂 : Kit _∋/⊢_ ⦄ ⦃ K : KitT 𝕂 ⦄ {µ₁ µ₂} (ϕ : µ₁ –[ 𝕂 ]→ µ₂) µ µ' →
