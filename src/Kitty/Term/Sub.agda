...
record Sub : Set₁ where
  ...
  field
    ...
    _·ₖ_ : ∀ ⦃ 𝕂 ⦄ {µ₁} {µ₂} {µ₃} → µ₁ –[ 𝕂 ]→ µ₂ → µ₂ –[ 𝕂 ]→ µ₃ → µ₁ –[ 𝕂 ]→ µ₃
  ...
  _~ₜ[_]_ ⦃ 𝕂₁ ⦄ ⦃ 𝕂₂ ⦄ {µ} {m/M₁} {m/M₂} x/t₁ eq x/t₂ = subst (µ ⊢_) eq (`/id' ⦃ 𝕂₁ ⦄ _ x/t₁) ≡  `/id' ⦃ 𝕂₂ ⦄ _ x/t₂
  ...
  ϕ₁ ~ ϕ₂ = ∀ m x → apₖ ϕ₁ m x ~ₜ apₖ ϕ₂ m x
  -- ϕ₁ ~ ϕ₂ = ∀ m x → `/id _ (apₖ ϕ₁ m x) ≡ `/id _ (apₖ ϕ₂ m x)
  ...
  ~'→~ : ∀ ⦃ 𝕂 ⦄ {µ₁} {µ₂} {ϕ₁ ϕ₂ : µ₁ –[ 𝕂 ]→ µ₂}
         → ϕ₁ ~' ϕ₂
         → ϕ₁ ~ ϕ₂
  ~'→~ ϕ₁~'ϕ₂ = λ m x → cong (`/id m) (ϕ₁~'ϕ₂ m x)

  ~ₜ-refl :
    ∀ ⦃ 𝕂 ⦄ {µ} {m} {x/t : µ ∋/⊢[ 𝕂 ] id/m→M m}
    → x/t ~ₜ x/t
  ~ₜ-refl = cong (`/id _) refl
...
record SubWithLaws : Set₁ where
  ...
  ι-~-→ ⦃ 𝕂₁ ⦄ ⦃ 𝕂₂ ⦄ ⦃ 𝕂₁⊑𝕂₂ ⦄ {µ₁} {µ₂} ϕ m x =
    let sub = subst (µ₂ ∋/⊢_) (ι-id/m→M m ) in
    `/id _ (apₖ (ι-→ ϕ) m x)         ≡⟨ cong (`/id _) (ι-ap-→ ϕ x) ⟩
    `/id _ (sub (ι-∋/⊢ (apₖ ϕ _ x))) ≡⟨ ι-∋/⊢-~ₜ (apₖ ϕ _ x) ⟩
    `/id _ (apₖ ϕ m x)               ∎
  ...
  ~-cong-,ₖ {µ₁ = µ₁} {µ₂} {.mx} {ϕ} {ϕ'} {x/t} {x/t'} ϕ~ϕ' x/t≡x/t' mx x@(here refl) =
    `/id _ (apₖ (ϕ ,ₖ x/t) mx x)   ≡⟨ cong (`/id _) (apₖ-,ₖ-here ϕ x/t) ⟩
    `/id _ (x/t)                   ≡⟨ x/t≡x/t' ⟩
    `/id _ (x/t')                  ≡⟨ cong (`/id _) (sym (apₖ-,ₖ-here ϕ' x/t')) ⟩
    `/id _ (apₖ (ϕ' ,ₖ x/t') mx x) ∎
  ~-cong-,ₖ {µ₁ = µ₁} {µ₂} {m} {ϕ} {ϕ'} {x/t} {x/t'} ϕ~ϕ' x/t≡x/t' mx x@(there y) =
    `/id _ (apₖ (ϕ ,ₖ x/t) mx x)   ≡⟨ cong (`/id _) (apₖ-,ₖ-there ϕ x/t y) ⟩
    `/id _ (apₖ ϕ mx y)            ≡⟨ ϕ~ϕ' mx y ⟩
    `/id _ (apₖ ϕ' mx y)           ≡⟨ cong (`/id _) (sym (apₖ-,ₖ-there ϕ' x/t' y)) ⟩
    `/id _ (apₖ (ϕ' ,ₖ x/t') mx x) ∎
  ...
  ~'-cong-,ₖ : ∀ ⦃ 𝕂 : Kit ⦄ {µ₁} {µ₂} {m} {ϕ ϕ' : µ₁ –[ 𝕂 ]→ µ₂} {x/t x/t' : µ₂ ∋/⊢[ 𝕂 ] id/m→M m} →
    ...
    x/t ≡ x/t' →
    (ϕ ,ₖ x/t) ~' (ϕ' ,ₖ x/t')
  ~'-cong-,ₖ ϕ~ϕ' x/t≡x/t' = ~→~' (~-cong-,ₖ (~'→~ ϕ~ϕ') (cong (`/id _) x/t≡x/t'))

  ~-cong-↓ : ∀ ⦃ 𝕂₁ 𝕂₂ : Kit ⦄ {µ₁} {µ₂} {m} {ϕ : (µ₁ ▷ m) –[ 𝕂₁ ]→ µ₂} {ϕ' : (µ₁ ▷ m) –[ 𝕂₂ ]→ µ₂} →
    ϕ ~ ϕ' →
    (ϕ ↓) ~ (ϕ' ↓)
  ~-cong-↓ ⦃ 𝕂 ⦄ {µ₁} {µ₂} {m} {ϕ} {ϕ'} ϕ~ϕ' mx x =
    `/id _ (apₖ (ϕ  ↓) mx x)     ≡⟨ cong (`/id _) (apₖ-↓ ϕ x) ⟩
    `/id _ (apₖ ϕ  mx (there x)) ≡⟨ ϕ~ϕ' mx (there x) ⟩
    `/id _ (apₖ ϕ' mx (there x)) ≡⟨ cong (`/id _) (sym (apₖ-↓ ϕ' x)) ⟩
    `/id _ (apₖ (ϕ' ↓) mx x)     ∎
