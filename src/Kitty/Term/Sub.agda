...
record Sub ℓ : Set (lsuc ℓ) where
  ...
  ~-cong-subst₂ :
    ∀ ⦃ 𝕂₁ 𝕂₂ ⦄ {µ₁} {µ₁'} {µ₂} {µ₂'} {f : µ₁ –[ 𝕂₁ ]→ µ₂} {g : µ₁ –[ 𝕂₂ ]→ µ₂} (eq₁ : µ₁ ≡ µ₁') (eq₂ : µ₂ ≡ µ₂')
    → f ~ g
...
record SubWithLaws ℓ : Set (lsuc ℓ) where
  ...
  field
    ...
    &-ι-→ : ∀ ⦃ 𝕂₁ 𝕂₂ ⦄ ⦃ 𝕂₁⊑𝕂₂ : 𝕂₁ ⊑ₖ 𝕂₂ ⦄ {µ₁} {µ₂} {m} (ϕ : µ₁ –[ 𝕂₁ ]→ µ₂) (x : µ₁ ∋ m)
            → x & (ι-→ ϕ) ≡ ι-∋/⊢ (x & ϕ)
  ...
  ∥-↓ : ∀ ⦃ 𝕂 ⦄ {µ₁₁ µ₁₂ µ₂ m} → (ϕ₁ : µ₁₁ –[ 𝕂 ]→ µ₂) → (ϕ₂ : (µ₁₂ ▷ m) –[ 𝕂 ]→ µ₂) →
    (ϕ₁ ∥ ϕ₂) ↓ ~ ϕ₁ ∥ (ϕ₂ ↓)
  ∥-↓ ⦃ 𝕂 ⦄ {µ₁₁} {µ₁₂} {µ₂} {m} ϕ₁ ϕ₂ mx x =
    `/id (x & ((ϕ₁ ∥ ϕ₂) ↓))                           ≡⟨ cong `/id (&-↓ (ϕ₁ ∥ ϕ₂) x) ⟩
    `/id (there x & (ϕ₁ ∥ ϕ₂))                         ≡⟨ ∥-▷ ϕ₁ ϕ₂ _ (there x) ⟩
    `/id (there x & (ϕ₁ ∥ (ϕ₂ ↓)) ,ₖ (here refl & ϕ₂)) ≡⟨ cong `/id (&-,ₖ-there (ϕ₁ ∥ (ϕ₂ ↓)) _ x) ⟩
    `/id (x & ϕ₁ ∥ (ϕ₂ ↓))                             ∎

  &-∥-here : ∀ ⦃ 𝕂 ⦄ {µ₁₁ µ₁₂ µ₂ m} → (ϕ₁ : µ₁₁ –[ 𝕂 ]→ µ₂) → (ϕ₂ : (µ₁₂ ▷ m) –[ 𝕂 ]→ µ₂) →
    here refl & (ϕ₁ ∥ ϕ₂) ≡ here refl & ϕ₂
  &-∥-here ⦃ 𝕂 ⦄ {µ₁₁} {µ₁₂} {µ₂} {m} ϕ₁ ϕ₂ =
    here refl & (ϕ₁ ∥ ϕ₂)                         ≡⟨ ~→~' (∥-▷ ϕ₁ ϕ₂) _ (here refl) ⟩
