...
-- -- --       sub (Kit.`/id' 𝕂₂ _ x/t ⋯ ϕ) ≡ Kit.`/id' 𝕂 _ (x/t ⋯ᶜ ϕ)
  ...
  wk-cancels-,ₖ-· :
    ...
    → (wkₖ _ id ·ₖ (ϕ ,ₖ x/t)) ~ ι-→ ϕ
  wk-cancels-,ₖ-· {µ₁} {µ₂} {m} ϕ x/t mx x =
    let sub₁ = subst (µ₂ ∋/⊢[ 𝕂₁⊔𝕂₂ ]_) (ι-id/m→M ⦃ 𝕂₁⊑𝕂₁⊔𝕂₂ ⦄ mx) in
    let sub₂ = subst (µ₂ ∋/⊢[ 𝕂₁⊔𝕂₂ ]_) (ι-id/m→M ⦃ 𝕂₂⊑𝕂₁⊔𝕂₂ ⦄ mx) in
    apₖ (wkₖ _ id ·ₖ (ϕ ,ₖ x/t)) mx x         ≡⟨ ap/⋯-·ₖ (wkₖ _ id) (ϕ ,ₖ x/t) x ⟩
    sub₁ (apₖ (wkₖ _ id) _ x ap/⋯ (ϕ ,ₖ x/t)) ≡⟨ cong (λ ■ → sub₁ (■ ap/⋯ (ϕ ,ₖ x/t))) (apₖ-wkₖ-wk id x) ⟩
    sub₁ (wk _ (apₖ id _ x) ap/⋯ (ϕ ,ₖ x/t))  ≡⟨ cong (λ ■ → sub₁ (wk ⦃ 𝕂₁ ⦄ _ ■ ap/⋯ (ϕ ,ₖ x/t))) (apₖ-id x) ⟩
    sub₁ (wk _ (id/` _ x) ap/⋯ (ϕ ,ₖ x/t))    ≡⟨ cong (λ ■ → sub₁ (■ ap/⋯ (ϕ ,ₖ x/t))) (wk-id/` _ x) ⟩
    sub₁ (id/` _ (there x) ap/⋯ (ϕ ,ₖ x/t))   ≡⟨ ap/⋯-ap (there x) (ϕ ,ₖ x/t) ⟩
    sub₂ (ι-∋/⊢ (apₖ (ϕ ,ₖ x/t) _ (there x))) ≡⟨ cong (λ ■ → sub₂ (ι-∋/⊢ ■)) (apₖ-,ₖ-there ϕ x/t x) ⟩
    sub₂ (ι-∋/⊢ (apₖ ϕ _ x))                  ≡⟨ sym (ι-ap-→ ϕ x) ⟩
    apₖ (ι-→ ϕ) mx x                          ∎
