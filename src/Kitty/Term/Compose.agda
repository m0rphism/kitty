...
record ComposeTraversal : Set₁ where
  ...
  record WkDistKit {𝕂 : Kit} (C₁ : ComposeKit 𝕂 kitᵣ 𝕂) (C₂ : ComposeKit kitᵣ 𝕂 𝕂) : Set₁ where
  ...
  apₖ-wk-↑' {µ₁} {µ₂} ϕ m x =
    ...
    -- wk _ (apₖ id _ x) ⋯ (ϕ ↑ m)                  ≡⟨ cong (λ ■ → wk ⦃ kitₛ ⦄ _ ■ ⋯ (ϕ ↑ m)) (apₖ-id x) ⟩
    -- wk _ (` x) ⋯ (ϕ ↑ m)                         ≡⟨⟩
    -- (` x) ⋯ᵣ wkₖ _ id ⋯ (ϕ ↑ m)                  ≡⟨ cong (_⋯ ϕ ↑ m) (⋯-var x (wkₖ _ id)) ⟩
    -- ` apₖ ⦃ 𝕂 = kitᵣ ⦄  (wkₖ _ id) _ x ⋯ (ϕ ↑ m) ≡⟨ cong (λ ■ → ` ■ ⋯ (ϕ ↑ m)) (apₖ-wkₖ-wk id x) ⟩
    -- ` there (apₖ ⦃ 𝕂 = kitᵣ ⦄  id _ x) ⋯ (ϕ ↑ m) ≡⟨ cong (λ ■ → (` there ■) ⋯ ϕ ↑ m) (apₖ-id x) ⟩
    -- ` there x ⋯ (ϕ ↑ m)                          ≡⟨ ⋯-var (there x) (ϕ ↑ m) ⟩
    -- `/id _ (apₖ (ϕ ↑ m) _ (there x))             ≡⟨ cong (`/id _) (apₖ-↑-there ϕ x) ⟩
    -- `/id _ (wk _ (apₖ ϕ _ x))                    ∎
