...
record ComposeKit ⦃ 𝕂₁ : Kit ⦄ : Set₁ where
  ...
  field
    ...
    ap/⋯-·ₖ : ∀ ⦃ 𝕊 : SubWithLaws ⦄ ⦃ 𝕂₂ : Kit ⦄ {µ₁} {µ₂} {µ₃} {m}
                ...
                (ϕ₂ : µ₂ –[ 𝕂₂ ]→ µ₃)
                (x : µ₁ ∋ m)
              → let instance _ = 𝕂₁⊔ 𝕂₂ in
                let instance _ = 𝕂₁⊑[𝕂₁⊔ 𝕂₂ ] in
...
ap/⋯-·ₖ : ∀ ⦃ 𝕊 : SubWithLaws ⦄ ⦃ 𝕊' : SubWithLaws ⦄ ⦃ 𝕂₂ : Kit ⦄ {µ₁} {µ₂} {µ₃} {m}
          → let 𝕊'' = SubWithLaws.SubWithLaws-Sub 𝕊' in
            (σ : µ₁ –[ kitₛ ⦃ 𝕊 = 𝕊 ⦄ ; 𝕊'' ]→ µ₂)
            (ϕ : µ₂ –[ 𝕂₂ ; 𝕊'' ]→ µ₃)
            (x : µ₁ ∋ m)
          → let instance _ = kitₛ ⦃ 𝕊 ⦄ in
            apₖ (σ ₛ· ϕ) _ x ≡ apₖ σ _ x ⋯ ϕ
ap/⋯-·ₖ ⦃ 𝕊 ⦄ ⦃ 𝕊' ⦄ ⦃ 𝕂₂ ⦄ {µ₁} {µ₂} {µ₃} {m} σ ϕ x =
  let instance _ = kitₛ ⦃ 𝕊 ⦄ in
  apₖ (σ ₛ· ϕ) _ x ≡⟨⟩
  apₖ (post σ (λ _ t → t ⋯ ϕ)) _ x ≡⟨ apₖ-post σ (λ _ t → t ⋯ ϕ) x ⟩
  apₖ σ _ x ⋯ ϕ    ∎

akitₛ : ⦃ 𝕊 : SubWithLaws ⦄ → ComposeKit ⦃ kitₛ ⦄
akitₛ = record
  { 𝕂₁⊔       = λ 𝕂₂ → kitₛ
  ; 𝕂₁⊑[𝕂₁⊔_] = λ 𝕂₂ →  ⊑ₖ-refl ⦃ kitₛ ⦄ 
  ; _ap/⋯_    = λ t ϕ → t ⋯ ϕ
  ; _·ₖ_      = _ₛ·_
  ; ap/⋯-·ₖ   = ap/⋯-·ₖ
  -- ; lem2 = λ 𝕂₂ → {!⊑ₖ-⊤ ⦃ 𝕂₂ ⦄!}
