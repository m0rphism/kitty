...
--   `/id _ (apₖ (⦅ x/t ⦆ ·[ C₁ ] ϕ) _ x)                               ≡⟨ cong (`/id _) (ap/⋯-·ₖ ⦅ x/t ⦆ ϕ x) ⟩
--   `/id _ (sub (apₖ ⦅ x/t ⦆ _ x ap/⋯ ϕ))                              ≡⟨ cong (λ ■ → `/id _(sub (■ ap/⋯ ϕ))) (~→~' (⦅⦆-,ₖ x/t) _ x) ⟩
--   `/id _ (sub (apₖ (id ,ₖ x/t) _ x ap/⋯ ϕ))                          ≡⟨ cong (λ ■ → `/id _ (sub (■ ap/⋯ ϕ))) (apₖ-,ₖ-here id x/t) ⟩
--   `/id _ (sub (x/t ap/⋯ ϕ))                                          ≡⟨ cong (`/id _) (sym (cancel-subst' (µ₂ ∋/⊢[ 𝕂 ]_) (ι-id/m→M m) (sub (x/t ap/⋯ ϕ)))) ⟩
--   `/id _ (sub'' (sub₁ (sub (x/t ap/⋯ ϕ))))                           ≡⟨ cong (λ ■ → `/id _ (sub'' ■)) (sym (ι-∋/⊢-id refl _)) ⟩
--   `/id _ (sub'' (ι-∋/⊢ (sub (x/t ap/⋯ ϕ))))                          ≡⟨ cong (λ ■ → `/id _ (sub'' (ι-∋/⊢ ■)))
--                                                                             (sym (apₖ-,ₖ-here id (sub (x/t ap/⋯[ C₁ ] ϕ)))) ⟩
--   `/id _ (sub'' (ι-∋/⊢ (apₖ ⦃ 𝕂 = 𝕂 ⦄ (id ,ₖ sub (x/t ap/⋯[ C₁ ] ϕ)) _ (here refl)))) ≡⟨ cong (λ ■ → `/id _ (sub'' (ι-∋/⊢ ■))) (sym (~→~' (⦅⦆-,ₖ (sub (x/t ap/⋯[ C₁ ] ϕ))) _ (here refl))) ⟩
--   `/id _ (sub'' (ι-∋/⊢ (apₖ ⦃ 𝕂 = 𝕂 ⦄ ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆ _ (here refl)))) ≡⟨ cong (`/id _) (sym (ap/⋯-ap ⦃ C₂ ⦄ (here refl)
--                                                                                                     ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆)) ⟩
--   `/id _ (sub' (id/` _ (here refl) ap/⋯ ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆))  ≡⟨ cong (λ ■ → `/id _ (sub' (■ ap/⋯ ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆)))
--                                                                               (sym (apₖ-↑-here ϕ)) ⟩ 
--   `/id _ (sub' (apₖ (ϕ ↑ m) _ x ap/⋯ ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆))     ≡⟨ cong (`/id _) (sym (ap/⋯-·ₖ (ϕ ↑ m) ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆ x)) ⟩
--   `/id _ (apₖ ((ϕ ↑ m) ·[ C₂ ] ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆) _ x)       ∎
...
--   let sub₂ = subst (µ₂ ∋/⊢[ 𝕂 ]_) (ι-id/m→M ⦃ 𝕂₂⊑𝕂₁⊔𝕂₂ ⦃ C₁ ⦄ ⦄ mx) in
--   `/id _ (apₖ (⦅ x/t ⦆ ·[ C₁ ] ϕ) _ x)                               ≡⟨ cong (`/id _) (ap/⋯-·ₖ ⦅ x/t ⦆ ϕ x) ⟩
--   `/id _ (sub' (apₖ ⦅ x/t ⦆ _ x ap/⋯ ϕ))                             ≡⟨ cong (λ ■ → `/id _ (sub' (■ ap/⋯[ C₁ ] ϕ))) (~→~' (⦅⦆-,ₖ x/t) _ x) ⟩
--   `/id _ (sub' (apₖ (id ,ₖ x/t) _ x ap/⋯ ϕ))                         ≡⟨ cong (λ ■ → `/id _ (sub' (■ ap/⋯[ C₁ ] ϕ))) (apₖ-,ₖ-there id x/t y) ⟩
--   `/id _ (sub' (apₖ id _ y ap/⋯ ϕ))                                  ≡⟨ cong (λ ■ → `/id _ (sub' (■ ap/⋯[ C₁ ] ϕ))) (apₖ-id y) ⟩
--   `/id _ (sub' (id/` _ y ap/⋯ ϕ))                                    ≡⟨ cong (`/id ⦃ 𝕂 ⦄ _) (ap/⋯-ap y ϕ) ⟩
--   `/id _ (sub₂ (ι-∋/⊢ ⦃ 𝕂₂⊑𝕂₁⊔𝕂₂ ⦃ C₁ ⦄ ⦄ (apₖ ϕ _ y)))              ≡⟨ cong (`/id ⦃ 𝕂 ⦄ _) (sym (ι-ap-→ ϕ y)) ⟩
--   `/id _ (apₖ (ι-→ ⦃ 𝕂₂⊑𝕂₁⊔𝕂₂ ⦃ C₁ ⦄ ⦄ ϕ) _ y)                       ≡⟨ ι-~-→ ϕ _ y ⟩
--   `/id _ (apₖ ϕ _ y)                                                 ≡⟨ {!!} ⟩
--   -- `/id _ (apₖ (ϕ ·[ C₂ ] id) _ y)                                    ≡⟨ {!!} ⟩
--   `/id _ (apₖ (wkₖ _ ϕ ·[ C₂ ] ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆) _ y)      ≡⟨ cong (`/id _) (ap/⋯-·ₖ ⦃ C₂ ⦄ (wkₖ _ ϕ) ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆ y) ⟩
--   `/id _ (sub'' (apₖ (wkₖ _ ϕ) _ y ap/⋯ ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆)) ≡⟨ cong (λ ■ → `/id _ (sub'' (■ ap/⋯ ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆)))
--                                                                              (apₖ-wkₖ-wk ϕ y) ⟩
--   `/id _ (sub'' (wk _ (apₖ ϕ _ y) ap/⋯ ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆))  ≡⟨ cong (λ ■ → `/id _ (sub'' (■ ap/⋯ ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆)))
--                                                                              (sym (apₖ-↑-there ϕ y)) ⟩ 
--   `/id _ (sub'' (apₖ (ϕ ↑ m) _ x ap/⋯ ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆))   ≡⟨ cong (`/id _) (sym (ap/⋯-·ₖ ⦃ C₂ ⦄ (ϕ ↑ m) ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆ x)) ⟩
--   `/id _ (apₖ ((ϕ ↑ m) ·[ C₂ ] ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆) _ x)      ∎
...
-- --     let sub = subst (µ₂ ∋/⊢[ 𝕂 ]_) (ι-id/m→M m) in
-- --   (⦅ x/t ⦆ ·[ C₁ ] ϕ) ~ ((ϕ ↑ m) ·[ C₂ ] ⦅ sub (x/t ap/⋯[ C₁ ] ϕ) ⦆)
-- -- dist-↑-⦅⦆-· ⦃ 𝕂₁ ⦄ ⦃ 𝕂₂ ⦄ ⦃ 𝕂 ⦄ ⦃ C₁ ⦄ ⦃ C₂ ⦄ {µ₁} {µ₂} {m} x/t ϕ = ~'→~ λ where
-- --   mx x@(here refl) →
-- --     let sub = subst (µ₂ ∋/⊢[ 𝕂 ]_) (ι-id/m→M m) in
-- --     let sub' = subst (µ₂ ∋/⊢[ 𝕂 ]_) (ι-id/m→M m) in
-- --     let sub'' = subst (µ₂ ∋/⊢[ 𝕂 ]_) (ι-id/m→M m) in
