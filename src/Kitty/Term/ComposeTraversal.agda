...
record ComposeTraversal : Set (lsuc ℓ) where
  ...
  -- &/⋯-wk-↑ₛ : ∀ ⦃ 𝕂 ⦄ {S₁} {S₂} {s} {st} {sx : Sort st}
  --               (t : S₁ ⊢ sx)
  --               (ϕ : S₁ –[ 𝕂 ]→ S₂)
  --             → wk _ (t ⋯ ϕ) ≡ wk _ t ⋯ (ϕ ↑ s)
  -- &/⋯-wk-↑ₛ ⦃ 𝕂 ⦄ {S₁} {S₂} {s} {st} {sx} t ϕ =
  --   wk _ (t ⋯ ϕ)                                        ≡⟨ {!!} ⟩
  --   t ⋯ ϕ ⋯ wkₖ ⦃ 𝕂 = kitᵣ ⦄ _ id                       ≡⟨ {!!} ⟩
  --   t ⋯ (ϕ ↑* []) ⋯ (wkₖ ⦃ 𝕂 = kitᵣ ⦄ _ id ↑* [])       ≡⟨⟩
  --   t ⋯* (wkₖ ⦃ 𝕂 = kitᵣ ⦄ _ id ∷ ϕ ∷ []) ↑** []        ≡⟨
  ...
  -- _∘ₛ_ : ∀ ⦃ 𝕂₂ : Kit ⦄ {S₁} {S₂} {S₃} → S₂ –[ 𝕂₂ ]→ S₃ → S₁ –[ kitₛ ]→ S₂ → S₁ –[ kitₛ ]→ S₃
  -- ϕ₂ ∘ₛ ϕ₁ = ϕ₁ ₛ· ϕ₂
...
--     t ⋯ ϕ₁' ⋯ ϕ₂'  ∎
  ...
  dist-⦅⦆ₛ-⋯ ⦃ 𝕂 ⦄ ⦃ W ⦄ ⦃ C₁ ⦄ ⦃ C₂ ⦄ ⦃ C₃ ⦄ {S₁} {S₂} {s} {st} {s'} t t' ϕ =
    ...
    t ⋯ (ϕ ↑ s) ⋯ ⦅ t' &/⋯[ C₁ ] ϕ ⦆ₛ ≡⟨ cong (λ ■ → t ⋯ ϕ ↑ s ⋯ ⦅ ■ ⦆ₛ) (&/⋯-⋯ t' ϕ) ⟩
    t ⋯ (ϕ ↑ s) ⋯ ⦅ t' ⋯ ϕ ⦆ₛ         ∎
