module Kitty.Experimental.Examples.SystemF-TypingKits.SubjectReduction where
...
open import Kitty.Experimental.Examples.SystemF-TypingKits.Definitions

ope-pres-⊢ : ∀ {E : µ₁ ⊢ M} {T : µ₁ ∶⊢ M} {ρ : µ₁ →ᵣ µ₂} →
  Γ₂ ∋* ρ ∶ Γ₁ →
  Γ₁ ⊢ E     ∶ T →
  Γ₂ ⊢ E ⋯ ρ ∶ T ⋯ ρ
ope-pres-⊢ {Γ₂ = Γ₂}     {ρ = ρ} ⊢ρ (τ-` refl)                 = τ-` (⊢ρ _)
ope-pres-⊢ {T = t₁ ⇒ t₂} {ρ = ρ} ⊢ρ (τ-λ ⊢e)                   = τ-λ (subst (_ ⊢ _ ∶_) (dist-↑-ren t₂ ρ) (ope-pres-⊢ (ope-keep' ⊢ρ) ⊢e))
ope-pres-⊢                       ⊢ρ (τ-Λ ⊢e)                   = τ-Λ (ope-pres-⊢ (ope-keep' ⊢ρ) ⊢e)
ope-pres-⊢                       ⊢ρ (τ-· ⊢e₁ ⊢e₂)              = τ-· (ope-pres-⊢ ⊢ρ ⊢e₁) (ope-pres-⊢ ⊢ρ ⊢e₂)
ope-pres-⊢               {ρ = ρ} ⊢ρ (τ-∙ {t₂ = t₂} {t = t} ⊢e) = subst (_ ⊢ _ ∶_) (sym (dist-⦅⦆ₛ-⋯ᵣ t₂ t ρ)) (τ-∙ (ope-pres-⊢ ⊢ρ ⊢e))
ope-pres-⊢                       ⊢ρ τ-𝕥                        = τ-𝕥
ope-pres-⊢                       ⊢ρ τ-𝕜                        = τ-𝕜
...
wk-pres-⊢ : ∀ {m'} {E : µ ⊢ M} {T : µ ∶⊢ M} (T' : µ ∶⊢ m→M m') →
  Γ₂         ⊢ E      ∶ T →
  (Γ₂ ,, T') ⊢ wk _ E ∶ wk _ T
wk-pres-⊢ {Γ₂ = Γ₂} T ⊢v =  ope-pres-⊢ (ope-drop' {T = T} (ope-id' {Γ = Γ₂})) ⊢v
