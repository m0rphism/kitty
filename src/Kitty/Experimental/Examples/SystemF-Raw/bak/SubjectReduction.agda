module Kitty.Experimental.Examples.SystemF-Raw.SubjectReduction where
...
open import Kitty.Experimental.Examples.SystemF-Raw.Definitions
open Kit {{...}}
...
-- sub-lift-id : {{K : Kit}} → {v : Term µ₁ m} {v' : Kit._◆_ K µ₂ m'} {σ : µ₁ –[ K ]→ µ₂} →
--   wk v ⋯ (σ ,ₖ v') ≡ v ⋯ σ
-- sub-lift-id {v = v} {v' = v'} {σ = σ} = {! assoc-⋯ᵣ-⋯ₛ v wk (σ ,ₖ v') !}
...
-- We only need this for types, i.e. where m = m' = 𝕥.
-- dist-⋯-⋯ : ∀ (t₂ : Term (m' ∷ µ₁) m) (t : Term µ₁ m') (σ : µ₁ →ₛ µ₂) →
--   t₂ ⋯ ⦅ t ⦆ ⋯ σ ≡ t₂ ⋯ liftₛ m' σ ⋯ ⦅ t ⋯ σ ⦆
-- dist-⋯-⋯ = {!!}
...
dist-⋯-⋯' t σ = fun-ext₂ λ where
  ...
  _ (there x) →
    σ _ x                      ≡⟨ sym (⋯-id (σ _ x)) ⟩
    σ _ x ⋯ σ-id               ≡⟨ sym (assoc-⋯ᵣ-⋯ₛ (σ _ x) wk (⦅ t ⋯ σ ⦆)) ⟩
    σ _ x ⋯ wk ⋯ ⦅ t ⋯ σ ⦆ ∎
...
-- just say Γ₂ ≡ Γ₁ ⋯* ρ because weamenings in ρ require us to fill the gaps
-- between the weamened Γ₁ types with new Γ₂ types (the `T` in the `ope-drop`
-- constructor).
-- Also arbitrary renamings would allow swapping types in the context which
-- could violate the telescoping (I thinm).
data OPE : µ₁ →ᵣ µ₂ → Ctx µ₁ → Ctx µ₂ → Set where
  ope-id : ∀ {Γ : Ctx µ} →
    OPE ρ-id Γ Γ
  ope-meep  : ∀ {ρ : µ₁ →ᵣ µ₂} {Γ₁ : Ctx µ₁} {Γ₂ : Ctx µ₂} {T : Type µ₁ m} →
    OPE  ρ       Γ₁        Γ₂ →
    OPE (ρ ↑ m) (Γ₁ ,, T) (Γ₂ ,, (T ⋯ₜ ρ))
...
-- TODO: worms equally well with m instead of 𝕧, but requires even more ⋯ₜ versions of ⋯ lemmas...
ope-pres-telescope : ∀ {ρ : µ₁ →ᵣ µ₂} (x : µ₁ ∋ 𝕧) →
  OPE ρ Γ₁ Γ₂ →
  wk-drop-∈ (ρ 𝕧 x) (Γ₂ (ρ 𝕧 x)) ≡ wk-drop-∈ x (Γ₁ x) ⋯ ρ
ope-pres-telescope x           ope-id         = sym (⋯-id _)
ope-pres-telescope (here refl) (ope-meep {ρ = ρ} {T = T} ope) = sym (dist-lift-ren T ρ)
ope-pres-telescope (there x)   (ope-meep {ρ = ρ} {Γ₁ = Γ₁} {Γ₂ = Γ₂} ope) =
...
ope-pres-⊢ {t = t₁ ⇒ t₂} {ρ = ρ} ope (τ-λ ⊢v)                   = τ-λ (subst (_ ⊢ _ ∶_) (dist-lift-ren t₂ ρ) (ope-pres-⊢ (ope-meep ope) ⊢v))
ope-pres-⊢                       ope (τ-Λ ⊢v)                   = τ-Λ (ope-pres-⊢ (ope-meep ope) ⊢v)
...
lift-⊢* {m = 𝕥} {σ = σ} T ⊢σ (here refl) = τ-𝕧
...
⊢*-σ-id : ∀ {Γ : Ctx µ} →
  Γ ⊢* σ-id ∶ Γ
⊢*-σ-id {µ = m ∷ µ} {Γ = Γ} {𝕥} x = τ-𝕧
⊢*-σ-id {µ = m ∷ µ} {Γ = Γ} {𝕧} x rewrite ⋯-id {{K = kitₛ}} (wk-telescope Γ x) = τ-` refl

vsub-pres-⊢ : ∀ {Γ : Ctx µ} {e₁ : Term (𝕧 ∷ µ) 𝕧} {e₂ : Term µ 𝕧} {t₁ t₂ : Type µ 𝕧} →
  Γ ,, t₁ ⊢ e₁ ∶ t₂ ⋯ wk →
  Γ ⊢ e₂ ∶ t₁ →
  Γ ⊢ e₁ ⋯ ⦅ e₂ ⦆ ∶ t₂
vsub-pres-⊢ {e₁ = e₁} {e₂ = e₂} {t₂ = t₂} ⊢e₁ ⊢e₂ =
  subst (_ ⊢ _ ∶_)
        (t₂ ⋯ wk ⋯ ⦅ e₂ ⦆ ≡⟨ sub-lift-id t₂ _ _ ⟩
         t₂ ⋯ σ-id        ≡⟨ ⋯-id t₂ ⟩
         t₂               ∎)
        (_ ⊢ e₁ ⋯ ⦅ e₂ ⦆ ∶ t₂ ⋯ wk ⋯ₜ ⦅ e₂ ⦆ by
         sub-pres-⊢ {σ = ⦅ e₂ ⦆}
...
tsub-pres-⊢ : ∀ {Γ : Ctx µ} {e : Term (𝕥 ∷ µ) 𝕧} {t₂ : Term (𝕥 ∷ µ) 𝕥} {t : Type µ 𝕧} →
  Γ ,, tt ⊢ e ∶ t₂ →
  Γ ⊢ t ∶ tt →
  Γ ⊢ e ⋯ ⦅ t ⦆ ∶ t₂ ⋯ ⦅ t ⦆
...
subject-reduction (τ-∙ (τ-Λ ⊢e))       β-Λ        = tsub-pres-⊢ ⊢e τ-𝕧
