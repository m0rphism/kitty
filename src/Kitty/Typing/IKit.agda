    ...
    Γ ∋/⊢ here refl & ϕ₁ ∥ ϕ₂ ∶ sub (wk-telescope (Γ₁ ▶▶ wk*-Ctx µ₁ Γ₂) (here refl) ⋯ ϕ₁ ∥ ϕ₂)
      by subst₂ (λ ■₁ ■₂ → Γ ∋/⊢ ■₁ ∶ sub ■₂)
           ...
           (wk-telescope Γ₂ (here refl) ⋯ ϕ₂                           ≡⟨⟩
            ...
            wkₛ _ (lookup Γ₂ (here refl)) ⋯ sub₂ (wkₖ* µ₁ (id {µ = []}) ↑* (µ₂ ▷ m₂)) ·[ ckitᵣ ] (ϕ₁ ∥ ϕ₂)  ≡⟨ sym (⋯-assoc _ _ (ϕ₁ ∥ ϕ₂)) ⟩
            wkₛ _ (lookup Γ₂ (here refl)) ⋯ᵣ sub₂ (wkₖ* µ₁ (id {µ = []}) ↑* (µ₂ ▷ m₂)) ⋯ (ϕ₁ ∥ ϕ₂) ≡⟨ cong (_⋯ ϕ₁ ∥ ϕ₂) (dist-subst-sub' _ _ (wkₛ _ (lookup Γ₂ (here refl))) (wkₖ* µ₁ (id {µ = []}) ↑* (µ₂ ▷ m₂))) ⟩
