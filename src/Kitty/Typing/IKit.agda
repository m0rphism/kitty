...
record IKit
  ...
  _∋↑*/⊢↑*_ {µ₁} {µ₂} {Γ₁} {Γ₂} {ϕ} ⊢ϕ {µ' ▷ m'} Γ' {.m'} (here refl) t refl =
    ...
    (subst₂ (λ ■₁ ■₂ → (Γ₂ ▶▶ (Γ' ⋯Ctx' (ϕ ↑* []))) ∋/⊢ ■₁ ∶ subst (_∶⊢_ ((µ' Data.List.++ µ₂) ▷ m')) (sym (id/m→M/id m')) ■₂)
            (id/` (here refl)           ≡⟨ sym (&-↑-here (ϕ ↑* µ')) ⟩
             here refl & ϕ ↑* µ' ↑ m'   ≡⟨ sym (use-~-hom (↑*-▷ µ' m' ϕ) _ (here refl)) ⟩
             here refl & ϕ ↑* (µ' ▷ m') ∎)
  ...
  wk-drop-∈' : (x : µ ∋ m) → (µ' ▷▷ drop-∈ x µ) ⊢ M → (µ' ▷▷ µ) ⊢ M
  wk-drop-∈' (here _)  t = wkₛ _ t
  wk-drop-∈' (there x) t = wkₛ _ (wk-drop-∈' x t)

  wk-telescope' : Ctx' µ' µ → µ ∋ m → µ' ▷▷ µ ∶⊢ m→M m
  wk-telescope' Γ x = wk-drop-∈' x (lookup' Γ x)

  infix   4  _∋'_∶_
  _∋'_∶_ : Ctx' µ' µ → µ ∋ m → µ' ▷▷ µ ∶⊢ m→M m → Set
  Γ ∋' x ∶ t = wk-telescope' Γ x ≡ t

  _∋*/⊢*_∶_via_ : ∀ {µ µ₁ µ₂} → Ctx µ₂ → µ₁ –[ 𝕂 ]→ µ₂ → Ctx' µ µ₁ → µ –[ 𝕂 ]→ µ₂ → Set
  _∋*/⊢*_∶_via_ {µ} {µ₁} {µ₂} Γ₂ ϕ Γ₁ ϕ' =
    ∀ {m₁} (x : µ₁ ∋ m₁) (t : µ ▷▷ µ₁ ∶⊢ m→M m₁) (⊢x : Γ₁ ∋' x ∶ t)
    → Γ₂ ∋/⊢ x & ϕ ∶ subst (µ₂ ∶⊢_) (sym (id/m→M/id m₁)) (t ⋯ (ϕ' ∥ ϕ) )

  postulate
    _⊢∥'_ : ∀ {µ µ₁ µ₂} {Γ : Ctx µ} {Γ₁ : Ctx µ₁} {Γ₂ : Ctx' µ₁ µ₂} {ϕ₁ : µ₁ –[ 𝕂 ]→ µ} {ϕ₂ : µ₂ –[ 𝕂 ]→ µ} →
      Γ ∋*/⊢* ϕ₁ ∶ Γ₁ →
      Γ ∋*/⊢* ϕ₂ ∶ Γ₂ via ϕ₁ →
      Γ ∋*/⊢* (ϕ₁ ∥ ϕ₂) ∶ Γ₁ ▶▶ Γ₂
  -- _⊢∥'_ {µ} {µ₁} {[]} {Γ} {Γ₁} {Γ₂} {ϕ₁} {ϕ₂} ⊢ϕ₁ ⊢ϕ₂ {mx} x _ refl =
  --   let sub = subst (_∶⊢_ µ) (sym (id/m→M/id mx)) in
  --   Γ ∋/⊢ x & ϕ₁ ∥ ϕ₂ ∶ sub (wk-telescope (Γ₁ ▶▶ Γ₂) x ⋯ ϕ₁ ∥ ϕ₂)
  --     by subst₂ (λ ■₁ ■₂ → Γ ∋/⊢ ■₁ ∶ sub ■₂)
  --               (use-~-hom (~-sym (∥-[] ϕ₁ ϕ₂)) mx x)
  --               (~-cong-⋯ _ (~-sym (∥-[] ϕ₁ ϕ₂)))
  --               (⊢ϕ₁ x _ refl)
  -- _⊢∥'_ {µ} {µ₁} {µ₂ ▷ m₂} {Γ} {Γ₁} {Γ₂} {ϕ₁} {ϕ₂} ⊢ϕ₁ ⊢ϕ₂ {mx} x@(here refl) _ refl =
  --   let sub = subst (_∶⊢_ µ) (sym (id/m→M/id mx)) in
  --   Γ ∋/⊢ x & ϕ₁ ∥ ϕ₂ ∶ sub (wk-telescope (Γ₁ ▶▶ Γ₂) x ⋯ ϕ₁ ∥ ϕ₂)
  --     by subst₂ (λ ■₁ ■₂ → Γ ∋/⊢ ■₁ ∶ sub ■₂)
  --               (sym (&-∥-here ϕ₁ ϕ₂))
  ...
  --               (⊢ϕ₂ (here refl) _ refl)
  -- _⊢∥'_ {µ} {µ₁} {µ₂ ▷ m₂} {Γ} {Γ₁} {Γ₂} {ϕ₁} {ϕ₂} ⊢ϕ₁ ⊢ϕ₂ {mx} x@(there y) _ refl =
  --   let sub = subst (_∶⊢_ µ) (sym (id/m→M/id mx)) in
  --   Γ ∋/⊢ x & ϕ₁ ∥ ϕ₂ ∶ sub (wk-telescope (Γ₁ ▶▶ Γ₂) x ⋯ ϕ₁ ∥ ϕ₂)
  --     by subst₂ (λ ■₁ ■₂ → Γ ∋/⊢ ■₁ ∶ sub ■₂)
  --               {!!}
...
record ITraversal : Set (lsuc ℓ) where
  ...
  field
    ...
    -- ⋯-var : ∀ {M} {_∋/⊢_ : Scoped M} ⦃ 𝕂 : Kit _∋/⊢_ ⦄ (x : µ₁ ∋ m) (f : µ₁ –→ µ₂) →
