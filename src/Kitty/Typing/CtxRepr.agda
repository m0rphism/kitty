...
record CtxRepr : Set₁ where
  ...
  dist-↓-▶▶'-~ :
    ∀ {µ₁ µ₂ µ₃ m} (Γ₁ : Ctx' µ₁ µ₂) (Γ₂ : Ctx' (µ₁ ▷▷ µ₂) (µ₃ ▷ m)) 
    → (Γ₁ ▶▶' Γ₂) ↓ᶜ ~ᶜ Γ₁ ▶▶' (Γ₂ ↓ᶜ)
  dist-↓-▶▶'-~ {µ₁} {µ₂} {µ₃} {m} Γ₁ Γ₂ mx x =
    let sub = subst (_∶⊢ m→M m) (sym (++-assoc µ₃ µ₂ µ₁)) in
  ...
  dist-↓-▶▶-~ {µ₂} {µ₃} {m} Γ₁ Γ₂ mx x =
    ...
    lookup' (Γ₁ ▶▶' (sub₁ Γ₂ ↓ᶜ)) x ≡⟨ cong (λ ■ → lookup' (Γ₁ ▶▶' ■) x) (dist-subst _↓ᶜ (sym (++-identityʳ µ₂)) Γ₂) ⟩
    lookup' (Γ₁ ▶▶' sub₂ (Γ₂ ↓ᶜ)) x ≡⟨⟩
    lookup' (Γ₁ ▶▶ Γ₂ ↓ᶜ) x         ∎
  ...
  module CtxReprSubst {ℓ} (𝕊 : SubWithLaws ℓ) (T : Traversal 𝕊) (H : KitHomotopy T) where
    ...
    infixl  5  _⋯Ctx'_
    _⋯Ctx'_ : ∀ {M} {_∋/⊢_ : Scoped M} ⦃ 𝕂 : Kit _∋/⊢_ ⦄ {µ₁ µ₂ µ'} → Ctx' µ₁ µ' → µ₁ –[ 𝕂 ]→ µ₂ → Ctx' µ₂ µ'
