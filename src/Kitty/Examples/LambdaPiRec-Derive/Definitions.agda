...
data _⊢_∶_ : Ctx S → S ⊢ s → S ∶⊢ s → Set where
  ...
  ⊢J : {Γ : Ctx S} {t : S ▷ 𝕖 ▷ 𝕖 ▷ 𝕖 ⊢ 𝕖} {ex ey t' : S ⊢ 𝕖}  →
    Γ ▶ t' ⊢ e₁ ∶ t ⋯ᵣ wkn {s = 𝕖} ⋯ₛ wkₖ 𝕖 (idₛ {S = S}) ,ₖ (` here refl) ,ₖ (` here refl) ,ₖ refl (` here refl) ,ₖ (` here refl) →
    Γ ⊢ e₂ ∶ (ex `≡ ey ∶ t') →
...
module Examples where
  ...
  `cong-suc : ∅ ▶ `ℕ ▶ `ℕ ▶ (# 1 `≡ # 0 ∶ `ℕ) ⊢ J (refl (`suc (# 0))) (# 0) ∶ (`suc (# 2) `≡ `suc (# 1) ∶ `ℕ) 
  `cong-suc = ⊢J {t = `suc (# 2) `≡ `suc (# 1) ∶ `ℕ} (⊢refl (⊢suc (⊢` refl))) (⊢` refl)
