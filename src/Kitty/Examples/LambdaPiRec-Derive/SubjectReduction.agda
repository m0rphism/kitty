module Kitty.Examples.LambdaPiRec-Derive.SubjectReduction where
...
open import Kitty.Examples.LambdaPiRec-Derive.Definitions
-- open import Kitty.Examples.LambdaPiRec-Derive.Confluence
...
open import Kitty.Typing.TypingKit compose-traversal ctx-repr
  record { _⊢_∶_ = _⊢_∶_ ; ⊢` = ⊢`; ≡ᶜ-cong-⊢ = λ { a b → {!!} } }
open TypingKit ⦃ … ⦄
...
merge-⦅⦆ :
  ∀ {e₁ : S ▷▷ (S' ▷ s') ⊢ s} {e₂ : S ▷▷ S' ▷▷ S'' ⊢ s'} →
  ⦅_⦆ₛ {S = S} {S' = S' ▷ s'} e₁  ·ₖ ⦅_⦆ₛ {S = S ▷▷ S'} {S' = S''}  e₂ ~
  subst ((S ▷ s) →ₛ_) (++-assoc S'' S' S) (⦅_⦆ₛ {S = S} {S' = S' ▷▷ S''} (e₁ ⋯ {!⦅_⦆ₛ {S = S} {S' = S' ▷▷ S''} ? !}))  
merge-⦅⦆ = {!!}
...
subject-reduction :
  ...
  e ↪ e' →
  Γ ⊢ e' ∶ t
subject-reduction (⊢· (⊢λ ⊢t₁ ⊢e₁) ⊢e₂) β-Π = ⊢e₁ ⊢⋯ₛ ⊢⦅ ⊢e₂ ⦆ₛ
subject-reduction (⊢· (⊢≣ x ⊢e) ⊢e₁) β-Π = {!!}
subject-reduction (⊢≣ x ⊢e) β-Π = {!!}
subject-reduction ⊢e β-Σ = {!!}
subject-reduction (⊢case (⊢ι #1 ⊢e) ⊢e₁ ⊢e₂) β-+ = {!!}
subject-reduction {Γ = Γ} (⊢case {e₁ = e₁} {e₂ = e₂} {t = t} (⊢ι {e = e} #2 ⊢e) ⊢e₁ ⊢e₂) β-+ =
  subst (Γ ⊢ e₂ ⋯ ⦅ e ⦆'ₛ ∶_)
        (t ⋯ ⦅ ι₂ (` here refl) ⦆ₛ ⋯ ⦅ e ⦆'ₛ               ≡⟨ ⋯-assoc t _ _ ⟩
         t ⋯ ⦅ ι₂ (` here refl) ⦆ₛ ·ₖ ⦅ e ⦆'ₛ              ≡⟨ ~-cong-⋯ t merge-⦅⦆' ⟩
         t ⋯ ⦅ ι₂ (` here refl) ⋯ ⦅ e ⦆'ₛ ⦆'ₛ              ≡⟨⟩
         t ⋯ ⦅ ι #2 e ⦆'ₛ                                  ∎)
        (⊢e₂ ⊢⋯ₛ ⊢⦅ ⊢e ⦆ₛ)
subject-reduction (⊢case (⊢≣ x ⊢e) ⊢e₁ ⊢e₂) β-+ = {!!}
subject-reduction (⊢≣ x ⊢e) β-+ = {!!}
subject-reduction {Γ = Γ} (⊢J {e₁ = e₁} {t = t} {ex = ex} ⊢e₁ (⊢refl ⊢e₂)) β-≡ =
  subst (Γ ⊢ e₁ ⋯ ⦅ ex ⦆'ₛ ∶_)
        (t ⋯ wkₖ 𝕖 id ,ₖ (` here refl) ,ₖ (` here refl) ,ₖ refl (` here refl) ⋯ ⦅ ex ⦆'ₛ ≡⟨ {!!} ⟩
         t ⋯ id ,ₖ ex ,ₖ ex ,ₖ refl ex ∎)
        (⊢e₁ ⊢⋯ₛ ⊢⦅ ⊢e₂ ⦆ₛ)
subject-reduction (⊢J ⊢e₁ (⊢≣ x ⊢e₂)) β-≡ = {!!}
subject-reduction (⊢≣ x ⊢e) β-≡ = {!!}
subject-reduction (⊢unfold (⊢fold ⊢e)) β-µ = ⊢e
subject-reduction (⊢unfold (⊢≣ x ⊢e)) β-µ = {!!}
subject-reduction (⊢≣ x ⊢e) β-µ = {!!}

subject-reduction ⊢e (ξ-Π₁ e↪e') = {!!}
subject-reduction ⊢e (ξ-Π₂ e↪e') = {!!}
subject-reduction ⊢e (ξ-λ e↪e') = {!!}
subject-reduction ⊢e (ξ-·₁ e↪e') = {!!}
subject-reduction ⊢e (ξ-·₂ e↪e') = {!!}
subject-reduction ⊢e (ξ-Σ₁ e↪e') = {!!}
subject-reduction ⊢e (ξ-Σ₂ e↪e') = {!!}
subject-reduction ⊢e (ξ-π e↪e') = {!!}
subject-reduction ⊢e (ξ-,₁ e↪e') = {!!}
subject-reduction ⊢e (ξ-,₂ e↪e') = {!!}
subject-reduction ⊢e (ξ-+₁ e↪e') = {!!}
subject-reduction ⊢e (ξ-+₂ e↪e') = {!!}
subject-reduction ⊢e (ξ-ι e↪e') = {!!}
subject-reduction ⊢e (ξ-case₁ e↪e') = {!!}
subject-reduction ⊢e (ξ-case₂ e↪e') = {!!}
subject-reduction ⊢e (ξ-case₃ e↪e') = {!!}
subject-reduction ⊢e (ξ-≡₁ e↪e') = {!!}
subject-reduction ⊢e (ξ-≡₂ e↪e') = {!!}
subject-reduction ⊢e (ξ-≡₃ e↪e') = {!!}
subject-reduction ⊢e (ξ-refl e↪e') = {!!}
subject-reduction ⊢e (ξ-J₁ e↪e') = {!!}
subject-reduction ⊢e (ξ-J₂ e↪e') = {!!}
subject-reduction ⊢e (ξ-µ e↪e') = {!!}
subject-reduction ⊢e (ξ-fold e↪e') = {!!}
subject-reduction ⊢e (ξ-unfold e↪e') = {!!}
