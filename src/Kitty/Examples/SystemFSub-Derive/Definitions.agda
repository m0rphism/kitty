...
⊑ₐ-trans :
  ...
  Γ ⊢ t₂ ⊑ₐ t₃ →
  Γ ⊢ t₁ ⊑ₐ t₃
⊑ₐ-trans (⊑ₐ-` {x = x} t₁⊑t₂ y t₁⊑t₃) t₂⊑t₃                        = ⊑ₐ-` {x = x} t₁⊑t₂ y (⊑ₐ-trans t₁⊑t₃ t₂⊑t₃)
⊑ₐ-trans ⊑ₐ-𝟙                         (⊑ₐ-` {x = x} t₂⊑t₃ y t₂⊑t₄) = ⊑ₐ-` {x = x} (⊑ₐ-trans ⊑ₐ-𝟙 t₂⊑t₃) y t₂⊑t₄
...
⊑ₐ-trans t₁⊑t₂                        ⊑ₐ-𝟙                         = ⊑ₐ-𝟙
⊑ₐ-trans t₁⊑t₂                        (⊑ₐ-` {x = x} t₂⊑t₃ y t₄⊑t₅) = ⊑ₐ-` {x = x} (⊑ₐ-trans t₁⊑t₂ t₂⊑t₃) y t₄⊑t₅
...
⊑ₐ→⊑ :
  Γ ⊢ t₁ ⊑ₐ t₂ → 
  Γ ⊢ t₁ ⊑ t₂
⊑ₐ→⊑ (⊑ₐ-` {x = x} t₁⊑ₐt₂ y t₂⊑ₐt₃) = ⊑-trans (⊑ₐ→⊑ t₁⊑ₐt₂) (⊑-trans (⊑-` {x = x} y) (⊑ₐ→⊑ t₂⊑ₐt₃))
⊑ₐ→⊑ ⊑ₐ-𝟙                           = ⊑-𝟙
