module Kitty.Examples.SystemFSub2-Derive.SubjectReduction where

open import Relation.Binary.PropositionalEquality using (_≡_; refl; sym; trans; subst; subst₂; cong; cong₂; module ≡-Reasoning)
open ≡-Reasoning
open import Kitty.Examples.SystemFSub2-Derive.Definitions
...
invert-⊑` : ∀ {Γ : Ctx S} {α : S ∋ 𝕥} →
  ...
  Γ ⊢ c ∶ t ⊑ (` α) →
  ∃[ β ] t ≡ ` β
invert-⊑` ⊢Γ (⊢` {x = x} ∋x) = ⊢Γ x ∋x
invert-⊑` ⊢Γ (⊑-trans ⊢c₁ ⊢c₂) with invert-⊑` ⊢Γ ⊢c₂
... | α , refl = invert-⊑` ⊢Γ ⊢c₁
...
invert-⊑⇒' : {Γ : Ctx S} →
  Valid Γ →
  Γ ⊢ c ∶ t ⊑ (t₁' ⇒ t₂') →
  (∃[ t₁ ] ∃[ t₂ ] Σ[ c₁ ∈ S ⊢ 𝕔 ] Σ[ c₂ ∈ S ⊢ 𝕔 ] t ≡ t₁ ⇒ t₂ × Γ ⊢ c₁ ∶ t₁' ⊑ t₁ × Γ ⊢ c₂ ∶ t₂ ⊑ t₂') ⊎ (∃[ α ] t ≡ ` α)
invert-⊑⇒' ⊢Γ (⊑-⇒ ⊢c₁ ⊢c₂) = inj₁ (_ , _ , _ , _ , refl , ⊢c₁ , ⊢c₂)
invert-⊑⇒' ⊢Γ (⊢` {x = x} ∋x) with ⊢Γ x ∋x
... | α , refl = inj₂ (α , refl)
invert-⊑⇒' ⊢Γ (⊑-trans ⊢c₁ ⊢c₂)
 with invert-⊑⇒' ⊢Γ ⊢c₂
... | inj₂ (α , refl) = inj₂ (invert-⊑` ⊢Γ ⊢c₁)
... | inj₁ (t₁' , t₂' , _ , _ , refl , ⊢c₁' , ⊢c₂')
 with invert-⊑⇒' ⊢Γ ⊢c₁
... | inj₂ (α , refl) = inj₂ (α , refl)
... | inj₁ (t₁'' , t₂'' , _ , _ , refl , ⊢c₁'' , ⊢c₂'') =
  inj₁ (_ , _ , _ , _ , refl , ⊑-trans ⊢c₁' ⊢c₁'' , ⊑-trans ⊢c₂'' ⊢c₂')

invert-⊑⇒ : {Γ : Ctx S} →
  Valid Γ →
  Γ ⊢ c ∶ (t₁ ⇒ t₂) ⊑ (t₁' ⇒ t₂') →
  Σ[ c₁ ∈ S ⊢ 𝕔 ] Σ[ c₂ ∈ S ⊢ 𝕔 ] Γ ⊢ c₁ ∶ (t₁' ⊑ t₁) × Γ ⊢ c₂ ∶ (t₂ ⊑ t₂')
invert-⊑⇒ ⊢Γ st with invert-⊑⇒' ⊢Γ st
... | inj₁ (_ , _ , _ , _ , refl , st₁ , st₂) = _ , _ , st₁ , st₂

-- TODO: Exactly the same proof as for ⇒
invert-⊑∀' : {Γ : Ctx S} {t₁' : S ⊢ 𝕥} {t₂' : S ▷ 𝕥 ⊢ 𝕥} →
  Valid Γ →
  Γ ⊢ c ∶ t ⊑ (∀[α⊑ t₁' ] t₂') →
  (∃[ t₁ ] ∃[ t₂ ] Σ[ c ∈ (S ▷ 𝕥) ⊢ 𝕔 ] t ≡ (∀[α⊑ t₁ ] t₂) × t₁ ≡ t₁' × Γ ▶ ★ ⊢ c ∶ t₂ ⊑ t₂') ⊎ (∃[ α ] t ≡ ` α)
invert-⊑∀' ⊢Γ (⊑-∀ ⊢c) = inj₁ (_ , _ , _ , refl , refl , ⊢c)
invert-⊑∀' ⊢Γ (⊢` {x = x} ∋x) with ⊢Γ x ∋x
... | α , refl = inj₂ (α , refl)
invert-⊑∀' ⊢Γ (⊑-trans ⊢c₁ ⊢c₂)
 with invert-⊑∀' ⊢Γ ⊢c₂
...
... | inj₂ (α , refl) = inj₂ (α , refl)
... | inj₁ (t₁'' , t₂'' , _ , refl , ⊢c₁'' , ⊢c₂'') =
  inj₁ (_ , _ , _ , refl , trans ⊢c₁'' ⊢c₁' , ⊑-trans ⊢c₂'' ⊢c₂')

invert-⊑∀ : {Γ : Ctx S} {t₁ t₁' : S ⊢ 𝕥} {t₂ t₂' : S ▷ 𝕥 ⊢ 𝕥} →
  Valid Γ →
  Γ ⊢ c ∶ (∀[α⊑ t₁ ] t₂) ⊑ (∀[α⊑ t₁' ] t₂') →
  Σ[ c ∈ S ▷ 𝕥 ⊢ 𝕔 ] t₁ ≡ t₁' × Γ ▶ ★ ⊢ c ∶ t₂ ⊑ t₂'
invert-⊑∀ ⊢Γ st with invert-⊑∀' ⊢Γ st
... | inj₁ (_ , _ , _ , refl , refl , st) = _ , refl , st
...
... | t₁ , t₂ , st , ⊢e₁'
 with invert-⊑⇒ ⊢Γ st
... | c₁ , c₂ , st₁ , st₂
 = let st₂' = subst (λ ■ → _ ⊢ _ ∶ ■ ⊑ _) (
                t₂                   ≡⟨ sym (wk-cancels-⦅⦆ t₂ e₂) ⟩
                t₂ ⋯ᵣ wkn ⋯ ⦅ e₂ ⦆'ₛ ∎
              ) st₂ in
   ⊢⊑ (⊢e₁' ⊢⋯ₛ ⊢⦅ ⊢⊑ ⊢e₂ st₁ ⦆ₛ) st₂'
subject-reduction {Γ = Γ} ⊢Γ (⊢∙ {t = t-bound} {t₁ = t-body} {t₂ = t-arg} {c = c} {e₁ = Λα e₁} ⊢t-body ⊢t-arg t-arg⊑t-bound ⊢e)   β-Λ
 with invert-Λ ⊢e
... | _ , t-body' , st , ⊢e'
 with invert-⊑∀ ⊢Γ st
... | c' , refl , t-body'⊑t-body
 = -- First we substitute the type variable at #1, which is under the constraint binding #0
   ...
   let ⊢e''' = Γ ⊢ e₁ ⋯ ⦅ t-arg ⦆ₛ ∶ t-body' ⋯ ⦅ t-arg ⦆ₛ
               by subst₂ (Γ ⊢_∶_) (wk-cancels-⦅⦆ (e₁ ⋯ ⦅ t-arg ⦆) c) (wk-cancels-⦅⦆ (t-body' ⋯ ⦅ t-arg ⦆) c) (
               Γ ⊢ e₁ ⋯ ⦅ t-arg ⦆ₛ ⋯ᵣ wknᵣ ⋯ ⦅ c ⦆ₛ ∶ t-body' ⋯ ⦅ t-arg ⦆ ⋯ᵣ wknᵣ ⋯ ⦅ c ⦆ₛ 
               by ⊢e'' ⊢⋯ₛ ⊢⦅ t-arg⊑t-bound ⦆ₛ
               ) in
