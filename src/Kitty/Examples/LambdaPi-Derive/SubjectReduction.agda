...
_↪*σ'_ : ∀ {µ₁ µ₂} (σ₁ σ₂ : µ₁ →ₛ µ₂) → Set
σ₁ ↪*σ' σ₂ = ∀ {m} (x : _ ∋ m) → σ₁ _ x ↪* σ₂ _ x

_↪ₚ*σ'_ : ∀ {µ₁ µ₂} (σ₁ σ₂ : µ₁ →ₛ µ₂) → Set
...
↪σ→↪ₚσ : ∀ {µ₁ µ₂} {σ σ' : µ₁ →ₛ µ₂} → σ ↪σ σ' → σ ↪ₚσ σ'
↪σ→↪ₚσ σ↪σ' x = ↪→↪ₚ (σ↪σ' x)

σ-[] : ∀ {µ} {σ σ' : [] →ₛ µ} → σ ≡ σ'
-- σ-[] = {!!}

↪*σ'→↪*σ : ∀ {µ₁ µ₂} {σ σ' : µ₁ →ₛ µ₂} → σ ↪*σ' σ' → σ ↪*σ σ'
↪*σ'→↪*σ {µ₁ = []}     σ↪σ' = {!!}
↪*σ'→↪*σ {µ₁ = µ₁ ▷ m} σ↪σ' = {!!}

↪ₚσ→↪*σ : ∀ {µ₁ µ₂} {σ σ' : µ₁ →ₛ µ₂} → σ ↪ₚσ σ' → σ ↪*σ σ'
↪ₚσ→↪*σ σ↪σ' = {!!} -- x  ↪ₚ→↪* (σ↪σ' x)

↪*σ→↪ₚ*σ : ∀ {µ₁ µ₂} {σ σ' : µ₁ →ₛ µ₂} → σ ↪*σ σ' → σ ↪ₚ*σ σ'
↪*σ→↪ₚ*σ σ↪*σ' = {!!} -- x   ↪*→↪ₚ* (σ↪*σ' x)

↪ₚ*σ→↪σ : ∀ {µ₁ µ₂} {σ σ' : µ₁ →ₛ µ₂} → σ ↪ₚ*σ σ' → σ ↪*σ σ'
↪ₚ*σ→↪σ σ↪σ' = {!!} -- x  ↪ₚ*→↪* (σ↪σ' x)
...
--   σ ↪σ σ' →
--   t ⋯ σ ↪* t' ⋯ σ'
-- ↪*σ-⋯ t↪t' σ↪σ' = ↪ₚ→↪* (↪σ-⋯ (↪→↪ₚ t↪t') (λ x → ↪→↪ₚ (σ↪σ' x)))

-- ↪σ-refl : ∀ {µ₁ µ₂} {σ : µ₁ →ₛ µ₂} →
--   σ ↪σ σ
-- ↪σ-refl {m = 𝕖} x = ↪-refl

-- ↪σ-ext : ∀ {µ₁ µ₂} {σ₁ σ₂ : µ₁ →ₛ µ₂} {m} {t₁ t₂ : µ₂ ⊢ m→M m} →
--   σ₁ ↪σ σ₂ →
--   t₁ ↪ t₂ →
--   (σ₁ ,ₛ t₁) ↪σ (σ₂ ,ₛ t₂)
...
--  with invert-≣-λ ∀≣∀
-- ... | t₁≣t₁' , t₂≣t₂'
--  =  ⊢≣ (≣-sym (≣-⋯ₛ t₂≣t₂' ≣σ-refl)) (⊢e₁ ⊢⋯ₛ ⊢⦅ ⊢≣ t₁≣t₁' ⊢e₂ ⦆ₛ) -- ⊢≣ {!≣-sym eq!} {!⊢e₁ ⊢⋯ₛ ⊢⦅ ⊢e₂ ⦆ₛ!}
-- subject-reduction (⊢· ⊢e₁ ⊢e₂) (ξ-·₁ e₁↪e₁') = ⊢· (subject-reduction ⊢e₁ e₁↪e₁') ⊢e₂
-- subject-reduction (⊢· ⊢e₁ ⊢e₂) (ξ-·₂ e₂↪e₂') = ⊢≣ (≣-⋯ₛ ≣-refl {!≣-↪ e₂↪e₂'!}) (⊢· ⊢e₁ (subject-reduction ⊢e₂ e₂↪e₂'))
