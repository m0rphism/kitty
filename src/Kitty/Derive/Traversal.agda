...
module Example where
  ...
  module Custom where
    ...
    --           → (t : (µ₁ ▷▷ µ₁' ▷▷ µ) ⊢ 𝕖)
    --           → let sub 𝕂s = subst₂ (_–[ 𝕂s ]→*_) (++-assoc µ µ₁' µ₁) (++-assoc µ µ₁' µ₂) in
    --             (foo {µ' = µ} t) ⋯* (f ↑** µ₁')
    --           ≡ foo {µ' = µ} (t ⋯* sub 𝕂s (f ↑** (µ₁' ▷▷ µ)))
    -- ⋯-↑-foo' {.[]}     []       t = {!refl!} -- refl
    ...
    ⋯-↑ {µ₁' = µ₁'} f g f≈g (λx t) =
      (λx t) ⋯* (f ↑** µ₁')           ≡⟨ ⋯-↑-λ f t ⟩
      λx (t ⋯* (f ↑** µ₁' ↑** [ 𝕖 ])) ≡⟨ cong λx_ (⋯-↑ (f ↑** µ₁') (g ↑** µ₁') (helper _⋯*_ f g f≈g) t) ⟩
      λx (t ⋯* (g ↑** µ₁' ↑** [ 𝕖 ])) ≡⟨ sym (⋯-↑-λ g t) ⟩
      (λx t) ⋯* (g ↑** µ₁')           ∎
    ⋯-↑ {µ₁' = µ₁'} f g f≈g (t₁ · t₂) =
      (t₁ · t₂) ⋯* (f ↑** µ₁')                                ≡⟨ ⋯-↑-· f t₁ t₂ ⟩
      (t₁ ⋯* (f ↑** µ₁' ↑** [])) · (t₂ ⋯* (f ↑** µ₁' ↑** [])) ≡⟨ cong₂ _·_ (⋯-↑ (f ↑** µ₁') (g ↑** µ₁') (helper _⋯*_ f g f≈g) t₁)
                                                                           (⋯-↑ (f ↑** µ₁') (g ↑** µ₁') (helper _⋯*_ f g f≈g) t₂) ⟩
      (t₁ ⋯* (g ↑** µ₁' ↑** [])) · (t₂ ⋯* (g ↑** µ₁' ↑** [])) ≡⟨ sym (⋯-↑-· g t₁ t₂) ⟩
      (t₁ · t₂) ⋯* (g ↑** µ₁')                                ∎
    ⋯-↑ {𝕂s₁} {𝕂s₂} {µ₁ = µ₁} {µ₂ = µ₂} {µ₁' = µ₁'} f g f≈g (foo {µ' = µ} t) =
      foo t ⋯* (f ↑** µ₁')                  ≡⟨ ⋯-↑-foo f t ⟩
      foo {µ' = µ} (t ⋯* (f ↑** µ₁' ↑** µ)) ≡⟨ cong foo (⋯-↑ (f ↑** µ₁') (g ↑** µ₁') (helper _⋯*_ f g f≈g) t) ⟩
      foo {µ' = µ} (t ⋯* (g ↑** µ₁' ↑** µ)) ≡⟨ sym (⋯-↑-foo g t) ⟩
      foo t ⋯* (g ↑** µ₁')                  ∎

    -- ⋯-↑ {µ₁' = µ₁'} f g f≈g (t₁ · t₂) =
    --   (t₁ · t₂) ⋯* (f ↑** µ₁')                  ≡⟨ ⋯-↑-·' f t₁ t₂ ⟩
    --   (t₁ ⋯* (f ↑** µ₁')) · (t₂ ⋯* (f ↑** µ₁')) ≡⟨ cong₂ _·_ (⋯-↑ f g f≈g t₁) (⋯-↑ f g f≈g t₂) ⟩
    --   (t₁ ⋯* (g ↑** µ₁')) · (t₂ ⋯* (g ↑** µ₁')) ≡⟨ sym (⋯-↑-·' g t₁ t₂) ⟩
    --   (t₁ · t₂) ⋯* (g ↑** µ₁')                  ∎
    -- ⋯-↑ {µ₁' = µ₁'} f g f≈g (λx t)    =
    --   (λx t) ⋯* (f ↑** µ₁')            ≡⟨ ⋯-↑-λ' f t ⟩
    --   λx (t ⋯* (f ↑** (µ₁' ▷▷ [ 𝕖 ]))) ≡⟨ cong λx_ (⋯-↑ f g f≈g t) ⟩
    --   λx (t ⋯* (g ↑** (µ₁' ▷▷ [ 𝕖 ]))) ≡⟨ sym (⋯-↑-λ' g t) ⟩
    --   (λx t) ⋯* (g ↑** µ₁')            ∎
    -- ⋯-↑ {𝕂s₁} {𝕂s₂} {µ₁ = µ₁} {µ₂ = µ₂} {µ₁' = µ₁'} f g f≈g (λx t)    =
    --   let sub 𝕂s = subst₂ (_–[ 𝕂s ]→*_) (++-assoc [ 𝕖 ] µ₁' µ₁) (++-assoc [ 𝕖 ] µ₁' µ₂) in
    --   (λx t) ⋯* (f ↑** µ₁')                    ≡⟨ ⋯-↑-λ f t ⟩
    --   λx (t ⋯* sub 𝕂s₁ (f ↑** (µ₁' ▷▷ [ 𝕖 ]))) ≡⟨ cong λx_ (⋯-↑ f g f≈g t) ⟩
    --   λx (t ⋯* sub 𝕂s₂ (g ↑** (µ₁' ▷▷ [ 𝕖 ]))) ≡⟨ sym (⋯-↑-λ g t) ⟩
    --   (λx t) ⋯* (g ↑** µ₁')                    ∎
    -- ⋯-↑ {𝕂s₁} {𝕂s₂} {µ₁ = µ₁} {µ₂ = µ₂} {µ₁' = µ₁'} f g f≈g (foo {µ' = µ} t) =
    --   let sub 𝕂s = subst₂ (_–[ 𝕂s ]→*_) (++-assoc µ µ₁' µ₁) (++-assoc µ µ₁' µ₂) in
    --   foo t ⋯* (f ↑** µ₁')                           ≡⟨ ⋯-↑-foo f t ⟩
    --   foo {µ' = µ} (t ⋯* sub 𝕂s₁ (f ↑** (µ₁' ▷▷ µ))) ≡⟨ {!!} ⟩
    --   foo {µ' = µ} (t ⋯* sub 𝕂s₂ (g ↑** (µ₁' ▷▷ µ))) ≡⟨ sym (⋯-↑-foo g t) ⟩
    --   foo t ⋯* (g ↑** µ₁') ∎
