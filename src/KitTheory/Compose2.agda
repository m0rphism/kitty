...
open import KitTheory.Kit2 using (KitTraversal)

module KitTheory.Compose2 {𝕄 : Modes} (𝕋 : Terms 𝕄) (T : KitTraversal 𝕋) where
...
open Modes 𝕄
open Terms 𝕋
open KitTheory.Kit2 𝕋
open KitTheory.Kit2.KitTraversal T
...
⋯-assoc' {{𝕂₁}} {{𝕂₂}} {{𝕂}} v f g =
  v ⋯ f ⋯ g                                  ≡⟨ refl ⟩
  v ⋯* (_∷_ {b = 𝕂₂} f (_∷_ {b = 𝕂₁} g []))
    ≡⟨ ⋯-↑ {µ = []} (_∷_ {b = 𝕂₂} f (_∷_ {b = 𝕂₁} g [])) (_∷_ {b = 𝕂} (g ∘ₖ f) []) (λ m₁ x →
      ` x ⋯* (_∷_ {b = 𝕂₂} f (_∷_ {b = 𝕂₁} g []))  ≡⟨ refl ⟩
