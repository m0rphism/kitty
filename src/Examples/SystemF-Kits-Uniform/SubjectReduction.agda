...
wk-⊢' {T = t₁ ⇒ t₂} {ρ = ρ} ope (τ-λ ⊢e)                          = τ-λ (wk-⊢' (ope-keep ope) ⊢e)
wk-⊢'                       ope (τ-Λ ⊢e)                          = τ-Λ (wk-⊢' (ope-keep ope) ⊢e)
wk-⊢'               {ρ = ρ} ope (τ-· {t₂ = t₂} {e₂ = e₂} ⊢e₁ ⊢e₂) = subst (_ ⊢ _ ∶_) (sym (dist-⦅⦆ₛ-⋯ᵣ t₂ e₂ ρ)) (τ-· (wk-⊢' ope ⊢e₁) (wk-⊢' ope ⊢e₂))
...
sub-𝕖-in-𝕥-id-var {µ' = .𝕥 ∷ µ'} (here refl) e₁ e₂ = refl
sub-𝕖-in-𝕥-id-var {µ' = m ∷ µ'}  (there α)   e₁ e₂ = cong (_⋯ wk) (sub-𝕖-in-𝕥-id-var α e₁ e₂)
